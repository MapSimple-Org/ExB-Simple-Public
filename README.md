...
## What's New in r024.62 (Feb 19, 2026)

### Service Error Feedback (r024.62)

**Query failures now surface user-facing messages.** When the backing ArcGIS map service is down or returns an unexpected response, a red-themed popover appears below the search form with: "Search could not be completed. The map service may be temporarily unavailable."

Previously, service outages caused silent failures: the spinner would disappear and the form would return with no feedback, leaving users confused about whether their search worked.

### Popup & Graphics Fixes (r024.58-61)

**r024.61 - Clear-All Race Condition Fix:**
- Fixed: "Remove all" (trash button) now properly clears graphics on first query in multi-widget apps
- Root cause: Concurrent layer creation during widget init created duplicate GraphicsLayers with same ID but different UIDs

**r024.59 - Popup Closes on Layer Toggle-Off:**
- In LayerList mode, toggling the result layer OFF now closes the popup (previously left orphaned popup pointing at nothing)

**r024.58 - Popup Persists in LayerList Mode:**
- Closing the widget panel no longer force-closes the popup when LayerList mode is enabled (graphics persist, so popup remains valid)

### Previous: LayerList Persistent Results (r024.0-18)

Query results can now be displayed as a `GroupLayer` visible in the LayerList and Legend widgets, persisting even when the widget is closed. See [RELEASE_v1.19.0-r024.18](docs/releases/RELEASE_v1.19.0-r024.18.md) for full details.
