...
- `query-simple/src/data-actions/zoom-to-action.tsx` — creates a `DataAction` that zooms the map to selected records.
- `query-simple/src/data-actions/index.tsx` — `getExtraActions()` aggregates and returns the widget's custom actions.

> Note (r028.080): Previous "Add to Map" / Select on map example was removed with the feature itself after its map-image sublayer highlight regression (BUG-SELECT-MAP-IMAGE-001) proved intractable on ExB 1.20.

**Use Actions in Your Component**
