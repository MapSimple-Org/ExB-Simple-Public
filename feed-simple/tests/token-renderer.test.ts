...
import { substituteTokens, resolveExternalLinkUrl } from '../src/utils/token-renderer'
...
// ── r027.092: HTML entity escaping (escapeHtml) ─────────────────

describe('substituteTokens — XSS prevention via escapeHtml', () => {
  it('should escape < and > in field values', () => {
    const item: FeedItem = { name: '<script>alert(1)</script>' }
    const result = substituteTokens('{{name}}', item)
    expect(result).not.toContain('<script>')
    expect(result).toContain('&lt;script&gt;alert(1)&lt;/script&gt;')
  })

  it('should escape ampersands in field values', () => {
    const item: FeedItem = { desc: 'Tom & Jerry' }
    const result = substituteTokens('{{desc}}', item)
    expect(result).toBe('Tom &amp; Jerry')
  })

  it('should escape double quotes in field values', () => {
    const item: FeedItem = { note: 'He said "hello"' }
    const result = substituteTokens('{{note}}', item)
    expect(result).toContain('&quot;hello&quot;')
  })

  it('should escape single quotes in field values', () => {
    const item: FeedItem = { note: "it's fine" }
    const result = substituteTokens('{{note}}', item)
    expect(result).toContain('it&#39;s fine')
  })

  it('should pass through safe text unchanged', () => {
    const item: FeedItem = { name: 'Normal text 123' }
    const result = substituteTokens('{{name}}', item)
    expect(result).toBe('Normal text 123')
  })

  it('should escape field values but preserve template text', () => {
    const item: FeedItem = { val: '<b>bold</b>' }
    const result = substituteTokens('Label: {{val}}', item)
    expect(result).toBe('Label: &lt;b&gt;bold&lt;/b&gt;')
  })
})

// ── r027.093: resolveExternalLinkUrl — dangerous URL blocking ───

describe('resolveExternalLinkUrl — dangerous URL blocking', () => {
  it('should return undefined for javascript: scheme from field value', () => {
    const item = { link: 'javascript:alert(document.cookie)' }
    const result = resolveExternalLinkUrl('{{link}}', item)
    expect(result).toBeUndefined()
  })

  it('should return undefined for data: scheme from field value', () => {
    const item = { url: 'data:text/html,<script>alert(1)</script>' }
    const result = resolveExternalLinkUrl('{{url}}', item)
    expect(result).toBeUndefined()
  })

  it('should return undefined for vbscript: scheme from field value', () => {
    const item = { url: 'vbscript:MsgBox("XSS")' }
    const result = resolveExternalLinkUrl('{{url}}', item)
    expect(result).toBeUndefined()
  })

  it('should block case-insensitive JAVASCRIPT:', () => {
    const item = { link: 'JAVASCRIPT:alert(1)' }
    const result = resolveExternalLinkUrl('{{link}}', item)
    expect(result).toBeUndefined()
  })

  it('should block whitespace-obfuscated java\\nscript:', () => {
    const item = { link: 'java\nscript:alert(1)' }
    const result = resolveExternalLinkUrl('{{link}}', item)
    expect(result).toBeUndefined()
  })
  ...
  it('should allow https: URLs', () => {
    const item = { id: '123' }
    const result = resolveExternalLinkUrl('https://example.com/detail/{{id}}', item)
    expect(result).toBe('https://example.com/detail/123')
  })

  it('should allow http: URLs', () => {
    const item = { id: '456' }
    const result = resolveExternalLinkUrl('http://example.com/{{id}}', item)
    expect(result).toBe('http://example.com/456')
  })

  it('should return undefined for empty template', () => {
    const result = resolveExternalLinkUrl(undefined, { id: '1' })
    expect(result).toBeUndefined()
  })
})
