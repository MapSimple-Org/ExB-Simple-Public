...
test.describe('QuerySimple: SQL Sanitization & Validation', () => {
  ...
  test('should exempt list-based queries from empty string rule', async ({ page }) => {
    ...
    // 1. Switch to a List-based query (Look for anything with "Regional" or "Unique" or "Trails")
    console.log('🔍 Searching for a List-based query in dropdowns...');
    try {
      // Try a few common names if Regional Trails isn't found
      const names = [/Regional Trails/i, /Unique/i, /Property/i];
      let found = false;
      for (const name of names) {
        try {
          await helpers.selectFromDropdown(name, 'alias', WIDGET_ID);
          found = true;
          break;
        } catch (e) {
          continue;
      ...
      if (!found) throw new Error('No list-based query found');
    } catch (e) {
      console.log('⚠️ Could not find a list-based query, skipping list exemption check');
      return;
    }
