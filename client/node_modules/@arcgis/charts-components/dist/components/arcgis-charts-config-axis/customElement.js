...
import { b as c, m as g, c as a, U as M, s as x } from "../../chunks/chart-ui-utils2.js";
...
import { LitElement as C, renderElement as I, safeClassMap as u } from "@arcgis/lumina";
...
class w extends C {
  constructor() {
    super(...arguments), this._messages = p(), this.commonStrings = p({ name: "common", blocking: !0 }), this.axisModel = new B(), this.minBoundInputElement = r(), this.maxBoundInputElement = r(), this.showLogScaleSwitch = r(), this.defaultMinBound = Number.NaN, this.defaultMaxBound = Number.NaN, this.onMaxBoundsChange = async () => {
      ...
      if (t !== void 0 && t !== "") {
        ...
        if (i !== this.axisModel.getMinBound() && c(i, n, this.maxBoundInputElement.value?.max)) {
          if (i !== this.axisModel.getMaxBound()) {
            this.axisModel.setMaxBound(i);
    ...
    }, this.onMaxBoundsChangeDebounced = m(this.onMaxBoundsChange, M.DebounceTimer), this.onMinBoundsChange = async () => {
      ...
      if (t !== void 0 && t !== "") {
        ...
        if (i !== this.axisModel.getMaxBound() && c(i, this.minBoundInputElement.value?.min, n)) {
          if (i !== this.axisModel.getMinBound()) {
            this.axisModel.setMinBound(i);
            const l = this.getAxisBoundsFromStateStore();
            l && (l.currMinBound = i);
          }
          e = a.IDLE;
    ...
    }, this.onMinBoundsChangeDebounced = m(this.onMinBoundsChange, M.DebounceTimer), this.onModelConfigChange = () => {
      this.requestUpdate();
  ...
  getAxisBoundsFromStateStore() {
    let t;
    const e = N.chartAxesBoundDataMap, n = this.model.chartId;
    if (n) {
      const i = f({
        chartId: n,
  ...
  async onShowLogScaleMouseEnter() {
    this.showLogScaleTooltip?.remove(), this.showLogScaleSwitch.value?.disabled && (this.showLogScaleTooltip = I(s`<calcite-tooltip .referenceElement=${this.showLogScaleSwitch.value ?? document.body} open>${this.commonStrings.utilsStrings.tooltips?.logDisabled ?? ""}</calcite-tooltip>`), document.body.appendChild(this.showLogScaleTooltip));
  }
  onShowLogScaleMouseLeave() {
    this.showLogScaleTooltip?.remove();
  }
  async onCategoryFormatChange(t) {
    this.axisModel.setValueFormat(t.target.categoryFormat);
  }
  async updateEnableLogarithmic(t) {
    t !== this.axisModel.getLogarithmic() && (this.axisModel.setLogarithmic(t), this.requestUpdate());
  }
  async onNumberFormatChange(t) {
    this.axisModel.setValueFormat({
  ...
  async onRevertMaxBoundsToDefault() {
    if (this.maxBoundInputElement.value?.status === a.INVALID || this.axisModel.getMaxBound() !== this.defaultMaxBound) {
      const t = this.getAxisBoundsFromStateStore();
      this.axisModel.getMinBound() > this.defaultMaxBound && this.axisModel.setMinBound(t?.defaultMinBound ?? Number.NaN), this.axisModel.setMaxBound(t?.defaultMaxBound ?? Number.NaN), this.requestUpdate();
    }
  }
  async onRevertMinBoundsToDefault() {
    (this.minBoundInputElement.value?.status === a.INVALID || this.axisModel.getMinBound() !== this.defaultMinBound) && (this.axisModel.getMaxBound() < this.defaultMinBound && this.axisModel.setMaxBound(Number.NaN), this.axisModel.setMinBound(Number.NaN), this.requestUpdate());
  }
  async enableLogScale(t) {
    const { checked: e } = t.target;
    this.axisModel.setLogarithmic(e && (this.dataMinValue ?? Number.NaN) > 0), this.requestUpdate();
  }
  render() {
    return s`<div><calcite-block .hidden=${this.axisModel.getValueFormat()?.type === d.DateAxisFormat} open .heading=${this.axisTitle}>${this.renderAxis()}</calcite-block></div>`;
  }
  renderAxis() {
    let t = null;
    const e = { ...this.axisModel.getValueFormat() };
    switch (this.axisModel.getValueFormat()?.type) {
      case d.CategoryAxisFormat:
...
b("arcgis-charts-config-axis", w);
export {
  w as ArcgisChartsConfigAxis
};
