...
import { n as d, i as c } from "../../chunks/series-types.js";
import { a as h } from "../../chunks/rest-js-object-literals.js";
import "@arcgis/toolkit/intl";
import { isEqual as v } from "lodash-es";
import "d3-array";
import { aX as F, aY as P, ci as x, bf as T, bd as C, cp as A } from "../../chunks/interfaces.js";
...
import { e as w, p as D } from "../../chunks/popover-ui-utils.js";
import { LitElement as E, createEvent as y, safeClassMap as o } from "@arcgis/lumina";
...
class B extends E {
  ...
  willUpdate(e) {
    ...
    const t = $.map((i) => F(i, this.commonStrings));
    v(this.aggregationLabels, t) || (this.aggregationLabels = t);
    const l = [d.Start, d.End].map((i) => P(i, this.commonStrings));
    v(this.timeAggregationLabels, l) || (this.timeAggregationLabels = l);
  }
  updated() {
    setTimeout(async () => {
      typeof this.picklist.value?.setFocus == "function" && await this.picklist.value?.setFocus();
    }, k.PopoverTimer), w(this.popoverElement.value, this.open), this.hasUpdated || setTimeout(() => {
  ...
  buildAggregationPickListItems() {
    return f($, (e) => e, (e, t) => n`<calcite-list-item .label=${this.aggregationLabels?.[t]} .value=${e} .selected=${u(e, this.selectedContent)}></calcite-list-item>`);
  }
  buildIntervalAlignmentPickListItems() {
    const e = [d.Start, d.End];
    return f(e, (t) => t, (t, s) => n`<calcite-list-item .label=${this.timeAggregationLabels?.[s]} .value=${t} .selected=${u(t, this.selectedContent)}></calcite-list-item>`);
  }
  buildPickListItem(e) {
    return b(e.name, n`<calcite-list-item .label=${x(this.layerFieldsInfo, e.name)} .value=${e.name} .selected=${u(e.name, this.selectedContent)}></calcite-list-item>`);
  }
  buildPickListItems(e) {
    ...
    switch (e) {
      ...
      case a.splitByField: {
        const l = b(" ", n`<calcite-list-item label=" " value=" " .selected=${u("", this.selectedContent)}></calcite-list-item>`);
        s.push(l);
        for (let i = 0; i < t; i += 1) {
          const p = this.layerFieldsInfo?.[i];
          p && (T(p) || p.type === h.String) && s.push(this.buildPickListItem(p));
        }
        break;
      }
    }
  ...
  renderXAxisField() {
    ...
    return n`<calcite-list label class=${o(r.pickListItemLabel)} selection-mode=single selection-appearance=border filter-enabled @calciteListChange=${this.onDataContentTypeChange} ${m(this.picklist)}>${e}</calcite-list>`;
  }
  renderNumericFields() {
    const e = this.buildPickListItems(a.numericFields);
    return n`<calcite-list label class=${o(r.numericFieldPickList)} selection-mode=multiple filter-enabled @calciteListChange=${this.onNumericFieldsChange} ${m(this.picklist)}>${e}</calcite-list>`;
  }
  renderSplitByField() {
  ...
  renderInterval() {
    ...
    return n`<div><div class=${o(r.sizeSection)}><calcite-label scale=m>${this._messages.intervalSize}<calcite-input type=number scale=m min=1 step=any .value=${N(e?.size, 1)} ${m(this.intervalSizeElement)}></calcite-input></calcite-label><calcite-label scale=m>${this._messages.intervalType}<calcite-select .label=${this._messages.intervalType ?? ""} ${m(this.intervalTypeElement)}>${t.map((s) => n`<calcite-option .value=${s} .label=${A(s, this._messages)} .selected=${e?.unit === s}></calcite-option>`)}</calcite-select></calcite-label><calcite-button class=${o(r.applyButton)} appearance=outline @click=${this.intervalChange}>${this._messages.apply}</calcite-button></div></div>`;
  }
  renderPopoverInfo() {
    let e;
