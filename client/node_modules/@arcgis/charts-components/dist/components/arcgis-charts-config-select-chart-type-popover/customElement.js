...
import { e as r } from "../../chunks/popover-ui-utils.js";
import { U as n } from "../../chunks/chart-ui-utils2.js";
import { u as p } from "../../chunks/useT9n.js";
import { createRef as h, ref as m } from "lit/directives/ref.js";
import { LitElement as d, createEvent as o, safeClassMap as s } from "@arcgis/lumina";
...
const i = {
  ...
  panel: "panel"
}, g = c`.hide{display:none}.am5-modal{width:100%;height:100%;position:absolute;z-index:100000;top:0;left:0}.am5-modal-curtain{top:0;left:0;width:100%;height:100%;position:absolute;background:#fff!important;z-index:100}.am5-modal-wrapper{top:0;left:0;width:100%;height:100%;position:absolute;display:flex;align-items:center;justify-content:center;white-space:nowrap;background:#ffffff80;z-index:101}.am5-modal-content{display:inline-block;padding:1.2em;vertical-align:middle;text-align:start;white-space:normal;background:#fff;border-radius:4px;box-shadow:#00000073 0 0 36px;color:#000}.am5-layer-1000{z-index:1000!important}.am5-layer-30{z-index:100!important}.arcgis-charts-modal{box-shadow:none!important}.arcgis-charts-modal-header{background-color:#0000000d;font-weight:700;padding:4px;align-content:center}.show{display:block}.notifyPanel{flex:0 1 auto}.disable-interactions{pointer-events:none}.dim-text{color:var(--arcgis-charts-dim-text)}.select-chart-component{width:var(--arcgis-charts-config-panel-width);height:var(--arcgis-charts-config-panel-height)}calcite-block{width:var(--arcgis-charts-config-calcite-block-default-width);height:var(--arcgis-charts-config-calcite-block-default-height);margin:var(--arcgis-charts-config-calcite-block-default-margin)}.label{display:flex;margin-left:1rem;margin-bottom:.5rem;justify-content:space-between}.header{margin:0}.panel{max-height:60vh}`;
class f extends d {
  constructor() {
    super(...arguments), this._messages = p(), this.selectChartTypePopover = h(), this.arcgisChartsConfigPopoverClose = o(), this.arcgisChartsConfigSelectChartTypeChange = o();
  }
  static {
    this.properties = { messageOverrides: 0, headerTitle: 3, layerId: 3, open: 7, referenceElement: 0, popoverPlacement: 3, selectedModelType: 3 };
  }
  static {
    this.styles = g;
  }
  ...
  updated() {
    r(this.selectChartTypePopover.value, this.open), this.hasUpdated || setTimeout(() => {
  ...
  popoverClose() {
    this.open = !1, this.arcgisChartsConfigPopoverClose.emit(), setTimeout(async () => await this.referenceElement?.setFocus(), 250);
  }
  render() {
    return a`<div class=${s({
      [i.selectChartComponent]: !0,
      [i.jsAppFlyout]: !0
    })}><calcite-popover .referenceElement=${this.referenceElement ?? document.body} .placement=${this.popoverPlacement} .flipPlacements=${["top", "bottom"]} @calcitePopoverOpen=${this.popoverOpen} @calcitePopoverClose=${this.popoverClose} .open=${this.open} auto-close label ${m(this.selectChartTypePopover)}><calcite-panel class=${s(i.panel)} .heading=${this.headerTitle ?? this._messages.chooseChartType} closable @calcitePanelClose=${this.popoverClose} .closed=${!this.open}>${this.buildChartTypesList()}</calcite-panel></calcite-popover></div>`;
  }
}
