...
import { b as d, m as r, s as p, c as m } from "../../chunks/chart-ui-utils2.js";
import { u as g } from "../../chunks/useT9n.js";
import { createRef as b, ref as f } from "lit/directives/ref.js";
import { LitElement as v, createEvent as x, safeClassMap as i } from "@arcgis/lumina";
...
class w extends v {
  ...
  onDecimalPlacesChange() {
    ...
    if (t !== void 0 && s && d(t, this.decimalPlacesInputElement.value?.min, this.decimalPlacesInputElement.value?.max)) {
      const c = Number.parseInt(t), e = {
      ...
      const o = this.getUpdatedNumberFormat(e);
      this.arcgisChartsConfigNumberFormatChange.emit(o), this.decimalPlacesInputElement.value && r({
  ...
  render() {
    return h`<div><div class=${i(n.container)}><calcite-label class=${i(n.label)}>${this._messages.decimalPlaces}<calcite-input type=number class=${i(n.numberInput)} min=0 max=20 step=1 .value=${p(this.numberFormat?.maximumFractionDigits, 2)} @calciteInputInput=${this.onDecimalPlacesChange} ${f(this.decimalPlacesInputElement)}></calcite-input></calcite-label><calcite-label layout=inline-space-between class=${i(n.label)}>${this._messages.showThousandSeparators}<calcite-switch name=useGrouping .checked=${!!(this.numberFormat?.useGrouping ?? !0)} @calciteSwitchChange=${this.onShowThousandsSeparatorChange}></calcite-switch></calcite-label></div></div>`;
  }
}
