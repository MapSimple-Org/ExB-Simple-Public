...
import { LitElement as f, createEvent as b, renderElement as x, safeClassMap as a, nothing as S } from "@arcgis/lumina";
...
class E extends f {
  ...
  loaded() {
    this.manager.onLifecycle(() => [
      g(() => [this._searchResultRendererEl, this.viewModel?.defaultPopupTemplate], () => {
        const e = this.viewModel?.defaultPopupTemplate, { _searchResultRendererEl: t } = this;
        if (e && t) {
          const i = document.createElement("div");
          i.appendChild(t), e.content = i;
        }
      }, { initial: !0 }),
      g(() => this.messages, (e) => this.viewModel.messages = e, { initial: !0 })
    ]);
  }
  ...
  _renderAutocomplete() {
    const { _effectiveActiveMenu: e, messages: t, _isDisabled: i } = this, { maxInputLength: r, placeholder: s, searchTerm: d, state: h } = this.viewModel, p = this.label ?? t.searchButtonTitle ?? "";
    return o`<calcite-autocomplete .autocomplete=${"off"} class=${a(c.autocomplete)} .disabled=${i} .icon=${this.icon || !1} .inputValue=${d} .label=${p} .loading=${h === "searching"} .maxLength=${r} .name=${this.el.id} @keydown=${this._handleInputKeydown} .open=${e === "suggestion"} overlay-positioning=fixed .placeholder=${s} title=${(d ? "" : s) ?? S} @calciteAutocompleteChange=${this._handleCalciteAutocompleteChange} @calciteAutocompleteClose=${this._handleAutocompleteClose} @calciteAutocompleteOpen=${this._handleAutocompleteOpen} @calciteAutocompleteTextChange=${this._handleCalciteAutocompleteTextInput} @calciteAutocompleteTextInput=${this._handleCalciteAutocompleteTextInput} ${w(this._handleAutocompleteCreate)}>${this._renderSuggestions()}${this._renderNotices()}</calcite-autocomplete>`;
  }
  _renderForm() {
    return o`<form class=${a(c.form)} @submit=${this._formSubmit} role=search>${this._renderAutocomplete()}<button class=${a(c.submitButton)} type=submit>${this.label ?? this.messages.searchButtonTitle ?? ""}</button></form>`;
  }
  ...
  _renderSuggestResults(e) {
    const { allSources: t, activeSourceIndex: i } = this.viewModel, { sourceIndex: r } = e, s = e.results?.length, d = t.length > 1 && i === -1, h = this._getSourceName(r);
    return s && d ? o`<calcite-autocomplete-item-group .heading=${h}>${this._renderSuggestGroup(e)}</calcite-autocomplete-item-group>` : this._renderSuggestGroup(e);
  }
