...
import { css as g, html as t } from "lit";
import { classes as p } from "@arcgis/toolkit/dom";
import h from "@arcgis/core/widgets/Print/PrintViewModel.js";
import { LitElement as m, createEvent as w, safeClassMap as s } from "@arcgis/lumina";
...
class b extends m {
  ...
  loaded() {
    ...
    const { height: e, width: i } = this.viewModel.templateOptions;
    this.viewModel.templateOptions.width = i || 800, this.viewModel.templateOptions.height = e || 1100;
  }
  ...
  render() {
    const e = {
      [a.panelItemsCentered]: this.viewModel.state === "initializing"
    };
    return t`<div class=${s(p(a.base, n.widget, n.panel, e))}>${this._renderPanel()}</div>`;
  }
  _renderPanel() {
    return this.viewModel.state === "initializing" ? t`<calcite-loader label=loading scale=m></calcite-loader>` : t`<calcite-flow><calcite-flow-item .selected=${!this._showTemplates}><div class=${s(a.printWidgetContainer)}>${this._renderPanelHeader()}${this._renderPanelContent()}</div></calcite-flow-item><calcite-flow-item .heading=${this.messages.chooseTemplate} .selected=${this._showTemplates} @calciteFlowItemBack=${(e) => {
  ...
  _renderPanelHeader() {
    return this.hideHeader ? null : t`<header class=${s(a.headerTitle)}>${this.messages.export}</header>`;
  }
  _renderPanelContent() {
    const e = this.viewModel.view, i = e?.type !== "2d";
    return !!this.error || i ? t`<div class=${s(a.panelError)}>${e ? i ? this.messages.sceneViewError : this.messages.serviceError : this.messages.noViewError}</div>` : t`<arcgis-print-main-panel .messages=${this.messages} .viewModel=${this.viewModel} .showTemplateSelector=${this._showTemplateSelector.bind(this)} .allowedFormatsForSaving=${this.allowedFormatsForSaving} .saveAsButtonCallback=${this.saveAsButtonCallback}></arcgis-print-main-panel>`;
  }
}
c("arcgis-print", b);
export {
