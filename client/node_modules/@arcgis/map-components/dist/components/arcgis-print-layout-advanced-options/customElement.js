...
import { LitElement as o, safeClassMap as l } from "@arcgis/lumina";
import { u as r } from "../../chunks/useHandles.js";
/*! All material copyright Esri, All Rights Reserved, unless otherwise specified.
See https://js.arcgis.com/4.34/esri/copyright.txt for details.
v4.34.3 */
const c = n`.arcgis-print__advanced-options-container{padding:var(--calcite-spacing-xxs);display:flex;flex-direction:column;gap:var(--calcite-spacing-xs)}`, h = "arcgis-print", p = {
  advancedOptionsContainer: `${h}__advanced-options-container`
};
class d extends o {
  constructor() {
    super(...arguments), this._handles = r("disconnect");
  }
  static {
  ...
  _renderAuthorSection() {
    return this._showAuthorInput ? i("author-info", t`<div><calcite-label>${this.messages.author}<calcite-input .value=${this.viewModel.templateOptions.author ?? ""} @calciteInputInput=${(e) => this.viewModel.templateOptions.author = e.target.value}></calcite-input></calcite-label></div>`) : null;
  }
  _renderCopyrightSection() {
    return this._showCopyrightInput ? i("copyright-text", t`<div><calcite-label>${this.messages.copyright}<calcite-input .value=${this.viewModel.templateOptions.copyright ?? ""} @calciteInputInput=${(e) => this.viewModel.templateOptions.copyright = e.target.value}></calcite-input></calcite-label></div>`) : null;
  }
  _renderDpiSection() {
  ...
  _renderLegendSection() {
    return this._showLegendInput ? t`<div><calcite-label layout=inline><calcite-checkbox .checked=${!!this.viewModel.templateOptions.legendEnabled} @calciteCheckboxChange=${(e) => this.viewModel.templateOptions.legendEnabled = e.target.checked}></calcite-checkbox>${this.messages.legend}</calcite-label></div>` : null;
  }
  ...
  _renderScaleBarSection() {
    return this._showScaleBarInput ? t`<div><calcite-label layout=inline><calcite-checkbox .checked=${!!this.viewModel.templateOptions.scaleBarEnabled} @calciteCheckboxChange=${(e) => this.viewModel.templateOptions.scaleBarEnabled = e.target.checked}></calcite-checkbox>${this.messages.scaleBar}</calcite-label></div>` : null;
  }
}
