...
class m extends b {
  ...
  get _traceResultsExist() {
    const { _traceResult: e } = this;
    return e !== void 0 && this._numTraceResults > 0;
  }
  get _numTraceResults() {
    const { _traceResult: e } = this;
    return e ? e.elements.length + e.globalFunctionResults.length : 0;
  }
  async destroy() {
    await this.manager.destroy();
  }
  _clearTraceResult() {
    this._traceResult = void 0;
  }
  _switchTab(e) {
    this._activeTab = e;
  }
  _getLoadError(e) {
    ...
    switch (e) {
      case "no-user-type-extension":
        return i.loadErrors.noUserTypeExtension;
      case "no-utility-network":
        return i.loadErrors.noUtilityNetwork;
      case "no-view":
        return i.loadErrors.noView;
      case "sceneView-not-supported":
        return i.loadErrors.sceneViewNotSupported;
      default:
        return e;
    }
  }
  async _runTrace() {
    const { viewModel: e, _selectedTraceConfig: i } = this;
    if (!i) {
      console.error(`${r} - Trace configuration not found`);
      return;
    }
    try {
      this._traceResult = await e.executeNamedTraceConfiguration({
        namedTraceConfigurationGlobalId: i.globalId
      });
    } catch (t) {
      console.error(`${r} - Error while running trace: `, String(t)), this._traceResult = void 0;
    }
  }
  _handleSubnetworkTraceConfigChange() {
    const { _selectedTraceConfig: e } = this;
    if (!e?.traceConfiguration) {
      console.error(`${r} - Trace configuration not found`);
      return;
    }
    const t = (e.traceConfiguration.subnetworkName ?? "") !== "";
    this._enableRunTraceButton = t;
  }
  _handleTraceConfigChange() {
    if (this._enableRunTraceButton = !1, this._selectedTraceConfig = this._traceInputsRef.value?.selectedTraceConfiguration, !this._selectedTraceConfig) {
      console.error(`${r} - Trace configuration not found`);
      return;
    }
    switch (this._selectedTraceConfig.traceType) {
      case "subnetwork":
        this._handleSubnetworkTraceConfigChange();
        break;
      default:
        console.error(`${r} - Unknown trace configuration type`);
    }
  }
  _handleTraceInputsChange() {
    this._handleTraceConfigChange();
  }
  _handleUtilityNetworkChange() {
    this.viewModel.utilityNetwork = this._traceInputsRef.value?.selectedUtilityNetwork;
  }
  renderResultsTab() {
    return n(`${this.el.id}__results-tab-item`, o`<calcite-tab .selected=${this._activeTab === "results"}>${this.viewModel.state === "executing" ? o`<calcite-loader .label=${this.messages.runningTrace} .text=${this.messages.runningTrace}></calcite-loader>` : o`<arcgis-utility-network-trace-results .layers=${this._allLayers} .traceConfigTitle=${this._selectedTraceConfig?.title} .traceResult=${this._traceResult} .utilityNetwork=${this.viewModel.utilityNetwork ?? void 0} ${s(this._traceResultsRef)}></arcgis-utility-network-trace-results>`}</calcite-tab>`);
  }
  renderResultsFooter() {
    const { _numTraceResults: e, _traceResultsExist: i } = this;
    return o`<calcite-button appearance=outline width=full @click=${() => {
      this._switchTab("input"), this._clearTraceResult();
    }} .disabled=${!i} slot=footer>${i ? this.messages.buttons.clearResultsCustom.replace("{number}", e.toString()) : this.messages.buttons.clearResultsGeneric}</calcite-button>`;
  }
  renderInputTab() {
    return n(`${this.el.id}__inputs-tab-item`, o`<calcite-tab .selected=${this._activeTab === "input"}><arcgis-utility-network-trace-inputs .view=${this.view || void 0} @arcgisTraceInputsChange=${this._handleTraceInputsChange} @arcgisUtilityNetworkChange=${this._handleUtilityNetworkChange} ${s(this._traceInputsRef)}></arcgis-utility-network-trace-inputs></calcite-tab>`);
  }
  renderInputFooter() {
    return o`<calcite-button appearance=solid width=full @click=${() => {
      this._switchTab("results"), this._runTrace();
    }} .disabled=${!this._enableRunTraceButton} slot=footer>${this.messages.buttons.getResults}</calcite-button>`;
  }
  render() {
    const { messages: e, viewModel: { loadError: i, state: t } } = this, l = t === "disabled" && i != null && i !== "no-utility-network" ? o`<calcite-notice kind=warning open scale=s slot=content-top width=full><div slot=message>${this._getLoadError(i)}</div></calcite-notice>` : void 0;
    return o`<div class="arcgis-utility-network-trace-analysis"><calcite-panel .loading=${this.view === void 0}><calcite-tabs layout=center position=top><calcite-tab-nav slot=title-group><calcite-tab-title @click=${() => {
      this._switchTab("input");
    }} .selected=${this._activeTab === "input"}>${e.tabs.inputTab}</calcite-tab-title><calcite-tab-title @click=${() => {
      this._switchTab("results");
    }} .selected=${this._activeTab === "results"}>${e.tabs.resultsTab}</calcite-tab-title></calcite-tab-nav>${this.view !== void 0 ? o`${this.renderInputTab()}${this.renderResultsTab()}` : null}</calcite-tabs>${l}${this._activeTab === "input" ? this.renderInputFooter() : null}${this._activeTab === "results" ? this.renderResultsFooter() : null}</calcite-panel></div>`;
  }
}
a("arcgis-utility-network-trace-analysis", m);
export {
  m as ArcgisUtilityNetworkTraceAnalysis
};
