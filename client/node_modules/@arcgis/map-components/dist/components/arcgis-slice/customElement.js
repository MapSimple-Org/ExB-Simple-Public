...
import { LitElement as L, createEvent as k, safeClassMap as a, nothing as M } from "@arcgis/lumina";
import { m as z } from "../../chunks/useViewModel.js";
import { A as S } from "../../chunks/AnalysisViewModel.js";
import E from "@arcgis/core/analysis/SliceAnalysis.js";
import { property as p, subclass as C } from "@arcgis/core/core/accessorSupport/decorators.js";
import { u as V } from "../../chunks/useT9n.js";
import { ignoreAbortErrors as G } from "@arcgis/core/core/promiseUtils.js";
...
const s = "arcgis-slice", r = {
  base: s,
  actions: `${s}__actions`,
  cancelButton: `${s}__cancel-button`,
  container: `${s}__container`,
  error: `${s}__error`,
  excludeButton: `${s}__exclude-button`,
  hint: `${s}__hint`,
  hintText: `${s}__hint-text`,
  layerItem: `${s}__layer-item`,
  layerList: `${s}__layer-list`,
  layerListHeading: `${s}__layer-list-title`,
  newSliceButton: `${s}__clear-button`
}, H = z(n);
class O extends L {
  constructor() {
    super(...arguments), this.viewModel = H(this), this._messages = V(), this.view = this.viewModel.view, this.analysis = this.viewModel.analysis, this.autoDestroyDisabled = !1, this.excludeGroundSurface = this.viewModel.excludeGroundSurface, this.excludedLayers = this.viewModel.excludedLayers, this.headingLevel = 3, this.icon = "slice", this.shape = this.viewModel.shape, this.state = this.viewModel.state, this.tiltEnabled = this.viewModel.tiltEnabled, this.arcgisPropertyChange = $()("analysis", "state"), this.arcgisReady = k();
  }
  static {
    this.properties = { _hasVoxelLayers: 16, view: 0, analysis: 0, autoDestroyDisabled: 5, excludeGroundSurface: 5, excludedLayers: 0, headingLevel: 9, icon: 3, label: 1, messageOverrides: 0, position: 1, referenceElement: 1, shape: 0, state: 35, tiltEnabled: 5 };
  }
  static {
    this.styles = j;
  }
  ...
  load() {
    this.manager.onLifecycle(() => [
      T(() => this._renderContainerNode(), () => this.requestUpdate())
    ]);
  }
  render() {
  ...
  _renderContainerNode() {
    const i = this._messages, { state: t, visible: d, supported: l } = this.viewModel, g = t === "slicing" || t === "excludingLayer";
    if (!d || !l)
      return null;
    l || o`<div class=${a(r.container)}><div class=${a(r.error)}><p>${this._messages.unsupported}</p></div></div>`;
    const { excludedLayers: u } = this.viewModel, f = this.excludeGroundSurface, v = u.length > 0 || f, h = t === "disabled";
    return o`<div class=${a(r.container)}><div class=${a(r.hint)}>${t === "excludingLayer" ? o`<p class=${a(r.hintText)}>${i.excludeHint}</p>` : null}${t === "slicing" ? o`<p class=${a(r.hintText)}>${this._hasVoxelLayers ? i.voxelHint : i.hint}</p><p class=${a(r.hintText)}>${i.verticalHint}</p>` : null}</div>${!g && v ? o`<div class=${a(r.layerList)}>${A({ class: r.layerListHeading, level: this.headingLevel, children: i.excludedLayers })}${P({ layers: u.toArray(), excludeGroundSurface: f, onLayerClick: (w) => {
      this.excludedLayers.remove(w);
    }, onGroundClick: () => {
      this.excludeGroundSurface = !1;
    }, listLabel: i.excludedLayers, layerActionLabel: i.includeLayer, groundItemTitle: i.ground })}</div>` : null}<div class=${a(r.actions)}>${g ? null : o`<calcite-button class=${a(r.newSliceButton)} .disabled=${h} @click=${this.start}>${i.newSlice}</calcite-button>`}${t === "sliced" ? o`<calcite-button appearance=outline-fill class=${a(r.excludeButton)} .disabled=${h} @click=${() => this.viewModel.pickLayerToExclude()}>${i.excludeLayer}</calcite-button>` : null}${t === "excludingLayer" ? o`<calcite-button appearance=outline-fill class=${a(r.cancelButton)} .disabled=${h} @click=${() => this.viewModel.stop()}>${i.cancel}</calcite-button>` : null}</div></div>`;
  }
}
function x(e) {
  return o`<calcite-list-item class=${a(r.layerItem)} data-testid=${`list-item-${e.uid}`} .label=${e.title ?? ""}><calcite-action icon=x @click=${e.onClick} scale=s slot=actions-end .text=${e.actionLabel} title=${e.actionLabel ?? M}></calcite-action></calcite-list-item>`;
}
function P(e) {
  const i = e.layers.map((t) => x({ uid: t.uid, title: t.title ?? "", onClick: () => e.onLayerClick(t), actionLabel: e.layerActionLabel }));
  return e.excludeGroundSurface && i.push(x({ uid: "ground", title: e.groundItemTitle, onClick: e.onGroundClick, actionLabel: e.layerActionLabel })), o`<calcite-list .label=${e.listLabel} selection-mode=none>${i}</calcite-list>`;
}
y("arcgis-slice", O);
export {
  O as ArcgisSlice
