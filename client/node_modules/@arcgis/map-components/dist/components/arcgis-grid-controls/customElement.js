...
import { LitElement as T, createEvent as P, safeClassMap as d, nothing as u } from "@arcgis/lumina";
...
class G extends T {
  ...
  get theme() {
    ...
    return !e || b.equals(e) ? "dark" : f.equals(e) ? "light" : "custom";
  }
  set theme(e) {
  ...
  loaded() {
    this.manager.onLifecycle(() => [
      ...
      D(() => this._viewModel.gridColor, (e) => {
        const i = e instanceof p ? e : typeof e == "string" ? p.fromHex(e) : null;
        i && !b.equals(e) && !f.equals(i) ? this.customColor = i : b.equals(i) ? this.theme = "dark" : f.equals(i) && (this.theme = "light");
      })
    ]);
  }
  ...
  render() {
    return r`<div class=${d(h(s.base, _.widget, _.panel))}><div class=${d(s.container)}>${this._renderContent()}</div></div>`;
  }
  _renderContent() {
    const { hideGridEnabledToggle: e, hidePlacementButtons: i, hideNumericInputs: t, hideLineIntervalInput: a, hideGridSnapEnabledToggle: n, hideDynamicScaleToggle: l, hideRotateWithMapToggle: c, hideColorSelection: g } = this;
    return [
      ...
      !n && this.snappingOptions ? this._renderSnappingToggle() : void 0,
      l ? void 0 : this._renderDynamicScaleToggle(),
      c ? void 0 : this._renderRotateToggle(),
      g ? void 0 : this._renderColorSelection()
    ];
  }
  _renderGridPlacementButtons() {
    const { interactivePlacementState: e, placementDisabled: i, gridControlsEnabled: t } = this, a = i || !t;
    return r`<calcite-label scale=s>${this._messages.placementOptions}<calcite-action-bar expand-disabled layout=horizontal>${W.map((n) => this._renderPlacementModeButton(n, a, e))}</calcite-action-bar></calcite-label>`;
  }
  _renderPlacementModeButton(e, i, t) {
    const a = this._messages?.placementState?.[e], n = this._placementModeButtonRefs.get(e);
    return r`<div><calcite-action .active=${t === e} alignment=center .disabled=${i} .icon=${m.placementState[e]} data-placement-action=${e ?? u} @click=${() => this._togglePlacementState(e)} .text=${a ?? ""} ${v(this._setPlacementButtonRef)}></calcite-action>${n ? r`<calcite-tooltip overlay-positioning=fixed placement=top .referenceElement=${n}>${a}</calcite-tooltip>` : void 0}</div>`;
  }
  _renderNumericInputs() {
    const { rotation: e, spacing: i, effectiveSpacingAfterDynamicScaling: t, numericSpacingInputShouldBeVisible: a, gridControlsEnabled: n } = this, l = t != null && t !== i;
    return r`<div class=${d(h(s.numericInputsContainer))}>${[
      a ? r`<calcite-label scale=s><span>${this._messages?.inputLabel?.spacing}${l ? r`<calcite-icon class=${d(h(s.inlineIcon))} .icon=${m.warning} scale=s tabindex=0 ${v(this._setWarningIconRef)}></calcite-icon>` : void 0}</span><calcite-input-number .disabled=${!n} number-button-type=none .value=${i?.toFixed(2)} @calciteInputNumberChange=${({ currentTarget: c }) => {
        const g = Number.parseFloat(c.value);
        this.spacing = g, g !== this.spacing && (c.value = `${this.spacing}`);
      }}></calcite-input-number>${l && this._warningIconTooltipRef ? r`<calcite-tooltip overlay-positioning=fixed placement=leading .referenceElement=${this._warningIconTooltipRef}>${R(this._messages?.warnings?.dynamicSpacing ?? "", { actualSpacingIncludingUnit: t }, {
        format: {
      ...
      r`<calcite-label scale=s>${this._messages?.inputLabel?.angle}<calcite-input-number .disabled=${!n} integer number-button-type=none suffix-text=\u00B0 .value=${`${L(e)}`} @calciteInputNumberChange=${({ currentTarget: c }) => this.rotation = z(c.value)}></calcite-input-number></calcite-label>`
    ]}</div>`;
  }
  _renderIntervalInput() {
    const { majorLineInterval: e, gridControlsEnabled: i } = this;
    return r`<div class=${d(h(s.numericInputsContainer))}><calcite-label scale=s>${this._messages?.inputLabel?.interval}<calcite-input-number .disabled=${!i} integer max=15 min=1 number-button-type=vertical .value=${`${e}`} @calciteInputNumberChange=${({ currentTarget: t }) => {
  ...
  _renderSnappingToggle() {
    ...
    return r`<arcgis-labeled-switch .checked=${!!t} .disabled=${!(i && !!e)} .label=${this._messages?.switchLabel?.snap} @arcgisCheckedChanged=${(n) => {
      n.stopPropagation(), this.snappingOptions?.set("gridEnabled", n.detail);
    }}></arcgis-labeled-switch>`;
  }
  _renderDynamicScaleToggle() {
    const { _messages: e, gridOutOfScale: i, dynamicScaling: t, gridControlsEnabled: a } = this, n = !this.hideOutOfScaleWarning && i;
    return r`<arcgis-labeled-switch .checked=${t} .disabled=${!a} .hint=${n ? e?.warnings?.outOfScale : void 0} .hintIcon=${m.warning} hint-kind=warning .label=${e?.switchLabel?.scaling} @arcgisCheckedChanged=${(l) => {
  ...
  _renderEnabledToggle() {
    return r`<div class=${d(s.borderedSwitch)}><arcgis-labeled-switch .checked=${this.displayEnabled} .disabled=${!!this.interactivePlacementState} .label=${this._messages?.switchLabel?.display} @arcgisCheckedChanged=${(e) => {
  ...
  _renderColorSelection() {
    ...
    return r`<calcite-label scale=s>${this._messages?.inputLabel?.theme}<div class=${d(s.tileGroup)}>${[
      ...
      this._themePopoverRef ? r`<calcite-popover auto-close .label=${this._messages?.gridLineColorPopoverLabel ?? ""} overlay-positioning=fixed .referenceElement=${this._themePopoverRef}><calcite-color-picker alpha-channel saved-disabled .value=${e?.toHex({ digits: 8 }) ?? "#000000ff"} @calciteColorPickerChange=${(i) => {
        const t = p.fromHex(i.currentTarget.value);
        t && (this.customColor = t, this.theme = "custom");
      }}></calcite-color-picker></calcite-popover>` : void 0
    ]}</div></calcite-label>`;
  }
  _renderThemeTile(e) {
    const { theme: i, gridControlsEnabled: t } = this, { value: a } = e, n = this._messages?.gridTheme?.[a], l = `${this._labelUid}--theme_tile_${a}`, c = `${l}__label`, g = m.gridTheme[a];
    return r`<div class=${d(h({ [s.tile]: !0, [s.tileDisabled]: !t }))} role=button ${v(a === "custom" ? this._setPopoverRef : this._emptyFunction)}><input aria-labelledby=${c ?? u} .checked=${i === a} .disabled=${!t} id=${l ?? u} .name=${`${this._labelUid}--theme`} @change=${(y) => {
      this.theme = a, y.currentTarget.focus();
    }} tabindex=0 type=radio><div class=${d(s.tileContent)}><calcite-icon .icon=${g} scale=l></calcite-icon><calcite-label .for=${l} id=${c ?? u}>${n}</calcite-label></div></div>`;
  }
}
$("arcgis-grid-controls", G);
export {
