...
import { LitElement as A, safeClassMap as h, nothing as o, safeStyleMap as M } from "@arcgis/lumina";
import { createRef as _, ref as v } from "lit/directives/ref.js";
import { g as I, a as w } from "../../chunks/action-utils.js";
...
function O({
  ...
  const i = t === "checkbox";
  return e ? a ? "circle-f" : i ? "check-square-f" : "view-visible" : a ? "circle" : i ? "square" : "view-hide";
}
function R({
  connectionStatus: a,
  publishing: e
}) {
  return a ? a === "connected" ? "beacon" : "offline" : e ? "square" : "bullet-point";
}
function P(a) {
  return a?.listMode === D.hideChildren;
}
function F(a) {
  const e = (a?.layer && k(a.layer) ? a.layer.layer : void 0) ?? a?.layer;
  return e ? e.type === "catalog" ? !1 : U(e) ?? !0 : !1;
}
function k(a) {
  return a != null && "layer" in a && a.layer != null;
}
function U(a) {
  const e = V(a);
...
function V(a) {
  return a && "capabilities" in a && a.capabilities != null && "exportMap" in a.capabilities ? a.capabilities.exportMap : void 0;
}
function B(a) {
  return a?.getAttribute("data-item");
}
function G(a) {
  const { children: e, error: t } = a, i = "incompatible" in a ? a.incompatible : !1;
  return !!e?.filter((n) => !n.hidden).length && !t && !i;
}
function N(a) {
  for (const e of a)
    for (const t of e)
...
function C(a, e) {
  const t = a.parent, i = a.layer;
  if (i?.type !== "map-image" || !i.sourceJSON || !t)
    return !1;
  const n = i.sourceJSON.layers;
  if (!n)
    return !1;
  const s = n.find((l) => a.id === l.id), c = e === "footprints" ? "Feature Layer" : "Catalog Dynamic Group Layer";
  return s?.type !== c ? !1 : n.find((l) => l.id === t.id)?.type === "Catalog Layer";
}
class H extends A {
  constructor() {
    super(...arguments), this._itemRef = _(), this._noticeRef = _(), this.messages = x(), this.hideCatalogLayerList = !1, this.showCloseButton = !1, this.showCollapseButton = !1, this.showErrors = !1, this.showFilter = !1, this.hideFlow = !1, this.showHeading = !1, this.hideStatusIndicators = !1, this.showTemporaryLayerIndicators = !1, this.showTemporaryTableIndicators = !1, this.dragEnabled = !1, this.listModeDisabled = !1, this.autoDestroyDisabled = !1;
  }
  static {
  ...
  get _renderedItemStatus() {
    const { item: e, parent: t, hideStatusIndicators: i, css: n } = this;
    if (!i)
      return null;
    const { publishing: s } = e, c = "updating" in e && e.updating && !t, l = "connectionStatus" in e ? e.connectionStatus : void 0, u = !!l;
    return r`<calcite-icon class=${h(y(n.statusIndicator, {
      [n.publishing]: s
    }, "updating" in n && {
      [n.updating]: c
    }, "connectionStatus" in n && {
      [n.connectionStatus]: u
    }, "connectionStatusConnected" in n && {
      [n.connectionStatusConnected]: u && l === "connected"
    }))} .icon=${R({ connectionStatus: l, publishing: s })} scale=s slot=content-end></calcite-icon>`;
  }
  get _renderedItemTemporaryIcon() {
    const { item: e, showTemporaryLayerIndicators: t, showTemporaryTableIndicators: i, css: n } = this, { layer: s } = e, c = s && "persistenceEnabled" in s && (S(s) || !s.persistenceEnabled);
    return (t || i) && c ? r`<calcite-icon class=${h(n.itemTemporaryIcon)} icon=temporary scale=s slot=content-start title=${this.messages.temporary ?? o}></calcite-icon>` : null;
  }
  get _renderedItemToggle() {
    const { item: e, parent: t, messages: i, visibilityAppearance: n, css: s } = this;
    if (!("visible" in e) || !("layerVisibility" in i) || !("visibleToggle" in s) || !n)
      return null;
    ...
    }), d = n === "checkbox", g = i.layerVisibility;
    return r`<calcite-action appearance=transparent class=${h(s.visibleToggle)} .icon=${d ? u : void 0} @click=${() => this._toggleVisibility(e, t)} scale=s .slot=${d ? "actions-start" : "actions-end"} .text=${g ?? ""} title=${g ?? o}>${d ? null : r`<calcite-icon class=${h(y({
      [s.visibleIcon]: l !== "exclusive" && c
  ...
  get _renderedPanelAction() {
    ...
    if (!e?.visible)
      return null;
    const { open: t, title: i, disabled: n } = e, s = {
      icon: e.icon,
      image: e.image,
      indicator: !1
    };
    return r`<calcite-action .active=${t} appearance=transparent .disabled=${n} .icon=${I(s)} @click=${() => this._togglePanel(e)} scale=s slot=actions-end .text=${i ?? ""} title=${i ?? void 0 ?? o}>${this._renderFallbackIcon(e)}</calcite-action>`;
  }
  ...
  get _renderedActionMenu() {
    const { item: e, messages: t } = this, i = t.options;
  ...
  get _renderedCatalogFootprintIcon() {
    const { css: e } = this, t = this.item.layer, i = t?.type === "catalog-footprint", n = t?.type === "sublayer" && C(t, "footprints");
    return "itemCatalogIcon" in e && (i || n) ? r`<calcite-icon class=${h(e.itemCatalogIcon)} icon=footprint scale=s slot=content-start></calcite-icon>` : null;
  }
  get _renderedCatalogDynamicIcon() {
    const { css: e } = this, t = this.item.layer, i = t?.type === "catalog-dynamic-group", n = t?.type === "sublayer" && C(t, "layers-in-view");
    return "itemCatalogIcon" in e && (i || n) ? r`<calcite-icon class=${h(e.itemCatalogIcon)} icon=catalog-dataset scale=s slot=content-start></calcite-icon>` : null;
  }
  get _renderedCatalogSelectNode() {
    const { _title: e, item: t, hideCatalogLayerList: i } = this;
    if (!("visible" in t))
      return;
    const { layer: n } = t, s = n?.type === "catalog-dynamic-group";
    return i && s ? r`<calcite-action appearance=transparent .disabled=${!n.visible} icon=chevron-right icon-flip-rtl @click=${() => this._triggerOnCatalogOpen(t)} scale=s slot=actions-end .text=${e}></calcite-action>` : null;
  }
  get _renderedChildList() {
    const { dragEnabled: e, item: t, rootGroupUid: i, listModeDisabled: n, selectionMode: s, displayMode: c } = this;
    if (!("children" in t))
      return;
    const l = [...this.parentTitles ?? [], t.title], { children: u, layer: d } = t, g = d?.type !== "catalog-dynamic-group" && G(t), f = n ? !1 : P(d), m = d?.type === "group", $ = !f && !g && e && m, b = e ? "childrenSortable" in t && t.childrenSortable && F(t) : !1;
    return g || $ ? r`<calcite-list .canPull=${(p) => this.canMove ? this.canMove(p, "pull") : !1} .canPut=${(p) => this.canMove ? this.canMove(p, "put") : !1} data-item=${t ?? o} data-layer-type=${i ?? o} .displayMode=${c} .dragEnabled=${b} .group=${m ? i : `${i}-${d?.uid}`} .label=${t.title} selection-appearance=border .selectionMode=${s}>${u?.filter((p) => !p.hidden && (this.showErrors || !p.error)).toArray().map((p) => this._renderItem(p, t, l))}${this._renderedTablesItem}</calcite-list>` : null;
  }
  get _hasTables() {
  ...
  get _renderedTablesItem() {
    ...
    return "itemTableIcon" in i && "visible" in e && "tables" in t && this._hasTables ? r`<calcite-list-item class=${h(i.item)} data-layer-id=${n?.id ?? o} drag-disabled .label=${t.tables} title=${t.tables ?? o} @calciteListItemSelect=${() => this._triggerOnTablesOpen(e)}><calcite-icon class=${h(i.itemTableIcon)} icon=table scale=s slot=content-start></calcite-icon><calcite-icon flip-rtl icon=chevron-right scale=s slot=content-end></calcite-icon></calcite-list-item>` : null;
  }
  ...
  _handleCalciteListItemSelect(e, t, i) {
    ...
    this.selectionMode !== "none" || n === "inherited" || this._toggleVisibility(t, i);
  }
  _togglePanel(e) {
  ...
  render() {
    ...
    return r`<calcite-list-item class=${h(y(d.item, "itemActive" in d && { [d.itemActive]: n === i }))} data-item=${i ?? o} data-layer-id=${m?.id ?? o} .dragDisabled=${!("sortable" in i && i.sortable) || f} id=${m?.uid ?? o} .label=${t} .metadata=${{ parentTitles: e, _title: t }} .expanded=${"open" in i && i.open} .selected=${s !== "none" ? c.includes(i) : !1} title=${g ?? o} .unavailable=${$} .value=${m?.uid} @calciteListItemSelect=${(b) => "visible" in i && this._handleCalciteListItemSelect(b, i, u)} @calciteListItemToggle=${(b) => "open" in i && this._handleCalciteListItemToggle(b, i)} ${v(this._itemRef)}>${this._renderedCatalogFootprintIcon}${this._renderedCatalogDynamicIcon}${this._renderedItemStatus}${this._renderedItemToggle}${this._renderedCatalogSelectNode}${this._renderedItemTemporaryIcon}${this._renderedChildList}${this._renderedItemMessage}${this._renderedPanel}${this._renderedPanelAction}${this._renderedActions}</calcite-list-item>`;
  }
  _renderAction(e) {
    const { item: t } = this, { action: i, textEnabled: n } = e;
    if (!i)
      return null;
    const { active: s, disabled: c, title: l, type: u, indicator: d } = i;
    return r`<calcite-action .active=${u === "toggle" ? i.value : !1} appearance=transparent data-action-id=${i.id ?? o} .disabled=${c} .icon=${I(i)} .indicator=${d} .loading=${s} @click=${() => this._triggerAction(t, i)} scale=s slot=${(n ? void 0 : "actions-end") ?? o} .text=${l ?? ""} .textEnabled=${n} title=${l ?? void 0 ?? o}>${this._renderFallbackIcon(i)}</calcite-action>`;
  }
  _renderFallbackIcon(e) {
    const { css: t } = this, { icon: i } = e;
    if (i)
      return null;
    const n = "image" in e ? e.image : void 0, s = {
      [t.itemActionImage]: !!n
    };
    return n ? r`<span aria-hidden=true class=${h(y(t.itemActionIcon, s))} style=${M(w(n))}></span>` : null;
  }
  _renderItem(e, t, i = []) {
...
T("arcgis-layer-list-item", H);
export {
  H as ArcgisLayerListItem
};
