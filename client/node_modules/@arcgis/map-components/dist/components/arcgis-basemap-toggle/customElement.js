...
import { getBasemapThumbnailUrl as z } from "@arcgis/core/applications/Components/basemapUtils.js";
import B from "@arcgis/core/widgets/BasemapToggle/BasemapToggleViewModel.js";
import { LitElement as M, createEvent as S, safeClassMap as i, nothing as c, safeStyleMap as u } from "@arcgis/lumina";
...
class V extends M {
  ...
  _getThumbnailStyles(t) {
    const s = z(t);
    return s ? { backgroundImage: `url(${s})` } : { backgroundImage: "" };
  }
  render() {
    const t = this.viewModel, s = t.state === "disabled" ? null : t.activeBasemap, r = t.state === "disabled" ? null : t.nextBasemap, v = t.state === "loading", g = t.state === "incompatible-next-basemap", d = r?.title ?? "", p = r && r.loadStatus !== "loaded";
    let m;
    const h = this.showTitle && d, _ = g, w = h || _, f = this._getThumbnailStyles(s), x = this._getThumbnailStyles(r);
    return w && (m = o`<div class=${i(e.overlay)}>${h ? o`<span class=${i(n(e.title, e.titleWrap))} title=${d ?? c}>${d}</span>` : null}${_ ? o`<calcite-scrim class=${i(e.overlayScrim)} title=${this.messages.incompatibleSpatialReference ?? c}><calcite-icon icon=exclamation-mark-triangle></calcite-icon></calcite-scrim>` : null}</div>`), o`<div class=${i(n(e.base, b.widget))}><calcite-button appearance=transparent data-basemap-id=${(r ? r.id : "") ?? c} .disabled=${g} kind=neutral .label=${this.label ?? void 0} @click=${() => void this.toggle()} title=${this.label ?? void 0 ?? c}><div class=${i(n(e.container, e.secondaryBasemapImage))}><div class=${i(e.image)} style=${u(f)}></div></div><div class=${i(e.container)}><div class=${i(n(e.image, p ? e.imageLoading : null))} style=${u(x)}>${p || v ? o`<calcite-scrim><span aria-hidden=true class=${i(b.loaderAnimation)} role=presentation></span></calcite-scrim>` : null}</div>${m}</div></calcite-button></div>`;
  }
}
y("arcgis-basemap-toggle", V);
export {
  V as ArcgisBasemapToggle
};
