...
const Wn = "Canceled";
function Ro(t) {
  return t instanceof Mo ? !0 : t instanceof Error && t.name === Wn && t.message === Wn;
}
class Mo extends Error {
  constructor() {
    super(Wn), this.name = this.message;
  }
}
...
(function(t) {
  ...
  t.slice = L;
  function F(C, x = Number.POSITIVE_INFINITY) {
    const w = [];
    if (x === 0)
      return [w, C];
...
class Tt {
  ...
  add(e) {
    ...
    return this._isDisposed ? Tt.DISABLE_DISPOSED_WARNING || console.warn(new Error("Trying to add a disposable to a DisposableStore that has already been disposed of. The added object will be leaked!").stack) : this._toDispose.add(e), e;
  }
  /**
...
const Vo = globalThis.performance && typeof globalThis.performance.now == "function";
class _n {
  static create(e) {
    return new _n(e);
  }
  constructor(e) {
    this._now = Vo && e === !1 ? Date.now : globalThis.performance.now.bind(globalThis.performance), this._startTime = this._now(), this._stopTime = -1;
  }
  stop() {
...
(function(t) {
  ...
  t.onceIf = i;
  function r(y, A, R) {
    return h((k, T = null, z) => y((X) => k.call(T, A(X)), null, z), R);
  }
  t.map = r;
  function s(y, A, R) {
    return h((k, T = null, z) => y((X) => {
      A(X), k.call(T, X);
    }, null, z), R);
  }
  t.forEach = s;
  function a(y, A, R) {
    return h((k, T = null, z) => y((X) => A(X) && k.call(T, X), null, z), R);
  }
  t.filter = a;
  function o(y) {
    return y;
  ...
  function l(...y) {
    return (A, R = null, k) => {
      const T = To(...y.map((z) => z((X) => A.call(R, X))));
      return d(T, k);
    };
  }
  t.any = l;
  ...
  function f(y, A, R = 100, k = !1, T = !1, z, X) {
    let le, de, rt, Vt = 0, wt;
    const So = {
      leakWarningThreshold: z,
      onWillAddFirstListener() {
        le = y((No) => {
          Vt++, de = A(de, No), k && !rt && ($t.fire(de), de = void 0), wt = () => {
            const ko = de;
            de = void 0, rt = void 0, (!k || Vt > 1) && $t.fire(ko), Vt = 0;
  ...
  class v {
    ...
    filter(A) {
      return this.steps.push((R) => A(R) ? R : F), this;
    }
    reduce(A, R) {
      let k = R;
  ...
  function U(y) {
    return (A, R, k) => {
      ...
      return k instanceof Tt ? k.add(le) : Array.isArray(k) && k.push(le), le;
    };
  }
...
class Si {
  ...
  constructor(e, n, i = (Si._idPool++).toString(16).padStart(3, "0")) {
...
class we {
  constructor(e) {
    this._size = 0, this._options = e, this._leakageMon = this._options?.leakWarningThreshold ? new Si(e?.onListenerError ?? _t, this._options?.leakWarningThreshold ?? $o) : void 0, this._perfMon = this._options?._profName ? new sn(this._options._profName) : void 0, this._deliveryQueue = this._options?.deliveryQueue;
  }
  dispose() {
  ...
  get event() {
    return this._event ??= (e, n, i) => {
      if (this._leakageMon && this._size > this._leakageMon.threshold ** 2) {
        ...
        console.warn(o);
        const l = this._leakageMon.getMostFrequentStack() ?? ["UNKNOWN stack", -1], c = new zo(`${o}. HINT: Stack shows most frequent listener (${l[1]}-times)`, l[0]);
        return (this._options?.onListenerError || _t)(c), nn.None;
      }
      if (this._disposed)
        return nn.None;
      n && (e = e.bind(n));
      const r = new Nn(e);
      let s;
      this._leakageMon && this._size >= Math.ceil(this._leakageMon.threshold * 0.2) && (r.stack = Ni.create(), s = this._leakageMon.check(r.stack, this._size + 1)), this._listeners ? this._listeners instanceof Nn ? (this._deliveryQueue ??= new Oo(), this._listeners = [this._listeners, r]) : this._listeners.push(r) : (this._options?.onWillAddFirstListener?.(this), this._listeners = r, this._options?.onDidAddFirstListener?.(this)), this._size++;
      const a = tn(() => {
        s?.(), this._removeListener(r);
      });
      return i instanceof Tt ? i.add(a) : Array.isArray(i) && i.push(a), a;
    }, this._event;
  }
  _removeListener(e) {
  ...
  _deliver(e, n) {
    if (!e)
      return;
    const i = this._options?.onListenerError || _t;
    if (!i) {
      e.value(n);
      return;
    }
   ...
   */
  fire(e) {
    if (this._deliveryQueue?.current && (this._deliverQueue(this._deliveryQueue), this._perfMon?.stop()), this._perfMon?.start(this._size), this._listeners) if (this._listeners instanceof Nn)
...
const jo = Wa() === "pseudo" || typeof document < "u" && document.location && document.location.hash.indexOf("pseudo=true") >= 0;
function Wi(t, e) {
  let n;
...
function Z(t, e, ...n) {
  return Wi(typeof t == "number" ? Go(t, e) : e, n);
}
function Go(t, e) {
  const n = Ho()?.[t];
  if (typeof n != "string") {
...
const ft = "en";
let On = !1, Hn = !1, kn = !1, Oa = !1, ki = !1, qt, Bn = ft, Oi = ft, Xo, Me;
const Pe = globalThis;
let Ee;
...
const Jo = typeof Ee?.versions?.electron == "string", Qo = Jo && Ee?.type === "renderer";
if (typeof Ee == "object") {
  On = Ee.platform === "win32", Hn = Ee.platform === "darwin", kn = Ee.platform === "linux", kn && Ee.env.SNAP && Ee.env.SNAP_REVISION, Ee.env.CI || Ee.env.BUILD_ARTIFACTSTAGINGDIRECTORY, qt = ft, Bn = ft;
  const t = Ee.env.VSCODE_NLS_CONFIG;
  if (t)
    try {
      const e = JSON.parse(t);
      qt = e.userLocale, Oi = e.osLocale, Bn = e.resolvedLanguage || ft, Xo = e.languagePack?.translationsConfigFile;
    } catch {
    }
  Oa = !0;
} else typeof navigator == "object" && !Qo ? (Me = navigator.userAgent, On = Me.indexOf("Windows") >= 0, Hn = Me.indexOf("Macintosh") >= 0, (Me.indexOf("Macintosh") >= 0 || Me.indexOf("iPad") >= 0 || Me.indexOf("iPhone") >= 0) && navigator.maxTouchPoints && navigator.maxTouchPoints > 0, kn = Me.indexOf("Linux") >= 0, Me?.indexOf("Mobi") >= 0, ki = !0, Bn = Wa() || ft, qt = navigator.language.toLowerCase(), Oi = qt) : console.error("Unable to resolve platform.");
const kt = On, Yo = Hn, Zo = Oa, Ko = ki, eu = ki && typeof Pe.importScripts == "function", tu = eu ? Pe.origin : void 0, Re = Me, nu = typeof Pe.postMessage == "function" && !Pe.importScripts;
(() => {
  if (nu) {
    const t = [];
...
const iu = !!(Re && Re.indexOf("Chrome") >= 0);
Re && Re.indexOf("Firefox") >= 0;
!iu && Re && Re.indexOf("Safari") >= 0;
Re && Re.indexOf("Edg/") >= 0;
...
class su {
  constructor(e, n) {
    this.lastCache = void 0, this.lastArgKey = void 0, typeof e == "function" ? (this._fn = e, this._computeKey = ru) : (this._fn = n, this._computeKey = e.getCacheKey);
  }
  get(e) {
...
class Xe {
  ...
  static {
    this.cache = new su({ getCacheKey: JSON.stringify }, (e) => {
      ...
      let a = e.filter((h) => !h.startsWith("_") && h in s);
  ...
  static {
    this._locales = new Hi(() => Object.keys(Xe.ambiguousCharacterData.value).filter((e) => !e.startsWith("_")));
  }
  static getLocales() {
...
if (typeof In < "u" && typeof In.process < "u") {
  ...
  // Supported
  get platform() {
    return kt ? "win32" : Yo ? "darwin" : "linux";
  },
  get arch() {
  },
  // Unsupported
...
const an = gt.cwd, gu = gt.env, pu = gt.platform, bu = 65, Du = 97, xu = 90, Cu = 122, Qe = 46, he = 47, De = 92, ze = 58, Eu = 63;
...
function Au(t, e) {
  if (t === null || typeof t != "object")
    throw new ja(e, "Object", t);
}
function te(t, e) {
  if (typeof t != "string")
    throw new ja(e, "string", t);
}
const Ve = pu === "win32";
function q(t) {
  return t === he || t === De;
      ...
      if (!(a === c - 1 || o === 1)) if (o === 2) {
        if (r.length < 2 || s !== 2 || r.charCodeAt(r.length - 1) !== Qe || r.charCodeAt(r.length - 2) !== Qe) {
          if (r.length > 2) {
            ...
            h === -1 ? (r = "", s = 0) : (r = r.slice(0, h), s = r.length - 1 - r.lastIndexOf(n)), a = c, o = 0;
            continue;
          } else if (r.length !== 0) {
            r = "", s = 0, a = c, o = 0;
            continue;
...
function Ga(t, e) {
  Au(e, "pathObject");
  const n = e.dir || e.root, i = e.base || `${e.name || ""}${wu(e.ext)}`;
...
const pe = {
    ...
    for (let r = t.length - 1; r >= -1; r--) {
      ...
      if (r >= 0) {
        if (s = t[r], te(s, `paths[${r}]`), s.length === 0)
          continue;
      } else e.length === 0 ? s = an() : (s = gu[`=${e}`] || an(), (s === void 0 || s.slice(0, 2).toLowerCase() !== e.toLowerCase() && s.charCodeAt(2) === De) && (s = `${e}\\`));
      const a = s.length;
      let o = 0, l = "", c = !1;
      ...
      if (a === 1)
        ...
        if (c = !0, q(s.charCodeAt(1))) {
          let d = 2, f = d;
          for (; d < a && !q(s.charCodeAt(d)); )
            d++;
          if (d < a && d !== f) {
            const m = s.slice(f, d);
            for (f = d; d < a && q(s.charCodeAt(d)); )
              d++;
            if (d < a && d !== f) {
              for (f = d; d < a && !q(s.charCodeAt(d)); )
                d++;
              (d === a || d !== f) && (l = `\\\\${m}\\${s.slice(f, d)}`, o = d);
            }
          }
        } else
          o = 1;
      else We(h) && s.charCodeAt(1) === ze && (l = s.slice(0, 2), o = 2, a > 2 && q(s.charCodeAt(2)) && (c = !0, o = 3));
      if (l.length > 0)
        if (e.length > 0) {
  ...
  relative(t, e) {
    ...
    if (f !== h) {
      ...
      a > h && (t.charCodeAt(r + f) === De ? d = f : f === 2 && (d = 3)), d === -1 && (d = 0);
    }
    let m = "";
    for (f = r + d + 1; f <= s; ++f)
      (f === s || t.charCodeAt(f) === De) && (m += m.length === 0 ? ".." : "\\..");
    return o += d, m.length > 0 ? `${m}${i.slice(o, l)}` : (i.charCodeAt(o) === De && ++o, i.slice(o, l));
  },
  toNamespacedPath(t) {
  ...
  parse(t) {
    ...
    if (q(r)) {
      if (i = 1, q(t.charCodeAt(1))) {
        let d = 2, f = d;
        for (; d < n && !q(t.charCodeAt(d)); )
          d++;
        if (d < n && d !== f) {
          for (f = d; d < n && q(t.charCodeAt(d)); )
            d++;
          if (d < n && d !== f) {
            for (f = d; d < n && !q(t.charCodeAt(d)); )
              d++;
    ...
    let s = -1, a = i, o = -1, l = !0, c = t.length - 1, h = 0;
    for (; c >= i; --c) {
      ...
      o === -1 && (l = !1, o = c + 1), r === Qe ? s === -1 ? s = c : h !== 1 && (h = 1) : s !== -1 && (h = -1);
    }
    return o !== -1 && (s === -1 || // We saw a non-dot character immediately before the dot
    h === 0 || // The (right-most) trimmed path component is exactly '..'
    h === 1 && s === o - 1 && s === a + 1 ? e.base = e.name = t.slice(a, o) : (e.name = t.slice(a, s), e.base = t.slice(a, o), e.ext = t.slice(s, o))), a > 0 && a !== i ? e.dir = t.slice(0, a - 1) : e.dir = e.root, e;
  },
  sep: "\\",
  delimiter: ";",
  win32: null,
  ...
  resolve(...t) {
    ...
    return e = on(e, !n, "/", Gn), n ? `/${e}` : e.length > 0 ? e : ".";
  },
  normalize(t) {
    if (te(t, "path"), t.length === 0)
      return ".";
    const e = t.charCodeAt(0) === he, n = t.charCodeAt(t.length - 1) === he;
    return t = on(t, !e, "/", Gn), t.length === 0 ? e ? "/" : n ? "./" : "." : (n && (t += "/"), e ? `/${t}` : t);
  },
  isAbsolute(t) {
  ...
  relative(t, e) {
    ...
    let h = "";
    for (c = n + l + 1; c <= i; ++c)
      (c === i || t.charCodeAt(c) === he) && (h += h.length === 0 ? ".." : "/..");
    return `${h}${e.slice(s + l)}`;
  },
  toNamespacedPath(t) {
    return t;
  },
  ...
  parse(t) {
    ...
    for (; l >= i; --l) {
      ...
      a === -1 && (o = !1, a = l + 1), h === Qe ? r === -1 ? r = l : c !== 1 && (c = 1) : r !== -1 && (c = -1);
    }
    if (a !== -1) {
      const h = s === 0 && n ? 1 : s;
      r === -1 || // We saw a non-dot character immediately before the dot
      c === 0 || // The (right-most) trimmed path component is exactly '..'
      c === 1 && r === a - 1 && r === s + 1 ? e.base = e.name = t.slice(h, a) : (e.name = t.slice(h, r), e.base = t.slice(h, a), e.ext = t.slice(r, a));
    }
...
let ke = class Ht {
  static isUri(e) {
    return e instanceof Ht ? !0 : e ? typeof e.authority == "string" && typeof e.fragment == "string" && typeof e.path == "string" && typeof e.query == "string" && typeof e.scheme == "string" && typeof e.fsPath == "string" && typeof e.with == "function" && typeof e.toString == "function" : !1;
  }
  /**
   * @internal
   */
  constructor(e, n, i, r, s, a = !1) {
    typeof e == "object" ? (this.scheme = e.scheme || Q, this.authority = e.authority || Q, this.path = e.path || Q, this.query = e.query || Q, this.fragment = e.fragment || Q) : (this.scheme = Nu(e, a), this.authority = n || Q, this.path = ku(this.scheme, i || Q), this.query = r || Q, this.fragment = s || Q, Su(this, a));
  }
  // ---- filesystem path -----------------------
  /**
  ...
  static parse(e, n = !1) {
    const i = Bu.exec(e);
    return i ? new st(i[2] || Q, zt(i[4] || Q), zt(i[5] || Q), zt(i[7] || Q), zt(i[9] || Q), n) : new st(Q, Q, Q, Q, Q);
  }
  /**
  ...
  static file(e) {
    let n = Q;
    if (kt && (e = e.replace(/\\/g, Fe)), e[0] === Fe && e[1] === Fe) {
      const i = e.indexOf(Fe, 2);
      i === -1 ? (n = e.substring(2), e = Fe) : (n = e.substring(2, i), e = e.substring(i) || Fe);
    }
  ...
  static joinPath(e, ...n) {
    ...
    let i;
    return kt && e.scheme === "file" ? i = Ht.file(pe.join(Xn(e, !0), ...n)).path : i = be.join(e.path, ...n), e.with({ path: i });
  }
  ...
  static revive(e) {
    if (e) {
      ...
      {
        const n = new st(e);
        return n._formatted = e.external ?? null, n._fsPath = e._sep === Xa ? e.fsPath ?? null : null, n;
      }
    } else return e;
  }
};
const Xa = kt ? 1 : void 0;
class st extends ke {
  constructor() {
...
function Iu(t) {
  ...
  for (let n = 0; n < t.length; n++) {
    const i = t.charCodeAt(n);
    i === 35 || i === 63 ? (e === void 0 && (e = t.substr(0, n)), e += Ja[i]) : e !== void 0 && (e += t[n]);
  }
  return e !== void 0 ? e : t;
}
function Xn(t, e) {
  let n;
  return t.authority && t.path.length > 1 && t.scheme === "file" ? n = `//${t.authority}${t.path}` : t.path.charCodeAt(0) === 47 && (t.path.charCodeAt(1) >= 65 && t.path.charCodeAt(1) <= 90 || t.path.charCodeAt(1) >= 97 && t.path.charCodeAt(1) <= 122) && t.path.charCodeAt(2) === 58 ? e ? n = t.path.substr(1) : n = t.path[1].toLowerCase() + t.path.substr(2) : n = t.path, kt && (n = n.replace(/\//g, "\\")), n;
}
function Jn(t, e) {
  const n = e ? Iu : ji;
  let i = "", { scheme: r, authority: s, path: a, query: o, fragment: l } = t;
  if (r && (i += r, i += ":"), (s || r === "file") && (i += Fe, i += Fe), s) {
    let c = s.indexOf("@");
    if (c !== -1) {
...
function Qa(t) {
  try {
    ...
    return t.length > 3 ? t.substr(0, 3) + Qa(t.substr(3)) : t;
  }
}
const Gi = /(%[0-9A-Za-z][0-9A-Za-z])+/g;
function zt(t) {
  return t.match(Gi) ? t.replace(Gi, (e) => Qa(e)) : t;
}
var Je;
(function(t) {
...
class Mu {
  ...
  rewrite(e) {
    ...
    return typeof s == "string" && (a += `&${Ru}=${encodeURIComponent(s)}`), ke.from({
      scheme: Ko ? this._preferredWebSchema : Je.vscodeRemoteResource,
      authority: `${i}:${r}`,
...
const Pu = new Mu(), Tu = "vscode-app";
class un {
  static {
    this.FALLBACK_AUTHORITY = Tu;
  }
  /**
   ...
   */
  uriToBrowserUri(e) {
    return e.scheme === Je.vscodeRemote ? Pu.rewrite(e) : (
      // ...only ever for `file` resources
      e.scheme === Je.file && // ...and we run in native environments
      (Zo || // ...or web worker extensions on desktop
      tu === `${Je.vscodeFileResource}://${un.FALLBACK_AUTHORITY}`) ? e.with({
        scheme: Je.vscodeFileResource,
        // We need to provide an authority here so that it can serve
        // as origin for network and loading matters in chromium.
        // If the URI is not coming with an authority already, we
        // add our own
        authority: e.authority || un.FALLBACK_AUTHORITY,
        query: null,
        fragment: null
      }) : e
    );
...
const Rn = "default", Uu = "$initialize";
class Vu {
...
class Wu {
  ...
  sendMessage(e, n, i) {
    ...
    return new Promise((s, a) => {
      this._pendingReplies[r] = {
        resolve: s,
        reject: a
      }, this._send(new Vu(this._workerId, r, e, n, i));
    });
  }
  listen(e, n, i) {
    let r = null;
    const s = new we({
      onWillAddFirstListener: () => {
        r = String(++this._lastSentReq), this._pendingEmitters.set(r, s), this._send(new $u(this._workerId, r, e, n, i));
      },
      onDidRemoveLastListener: () => {
        this._pendingEmitters.delete(r), this._send(new zu(this._workerId, r)), r = null;
      }
    });
    return s.event;
  }
  ...
  createProxyToRemoteChannel(e, n) {
    const i = {
      get: (r, s) => (typeof s == "string" && !r[s] && (Ka(s) ? r[s] = (a) => this.listen(e, s, a) : Za(s) ? r[s] = this.listen(e, s, void 0) : s.charCodeAt(0) === 36 && (r[s] = async (...a) => (await n?.(), this.sendMessage(e, s, a)))), r[s])
    };
  ...
  _handleRequestMessage(e) {
    ...
    this._handler.handleMessage(e.channel, e.method, e.args).then((r) => {
      this._send(new Ji(this._workerId, n, r, void 0));
    }, (r) => {
      r.detail instanceof Error && (r.detail = zi(r.detail)), this._send(new Ji(this._workerId, n, void 0, zi(r)));
    });
  }
  _handleSubscribeEventMessage(e) {
    const n = e.req, i = this._handler.handleEvent(e.channel, e.eventName, e.arg)((r) => {
      this._send(new qu(this._workerId, n, r));
    });
...
class Ou {
  constructor(e, n) {
    this._localChannels = /* @__PURE__ */ new Map(), this._remoteChannels = /* @__PURE__ */ new Map(), this._requestHandlerFactory = n, this._requestHandler = null, this._protocol = new Wu({
      sendMessage: (i, r) => {
        e(i, r);
      },
  ...
  _handleMessage(e, n, i) {
    ...
    const r = e === Rn ? this._requestHandler : this._localChannels.get(e);
    if (!r)
...
function ju(t, e, n) {
  return new Ge(new Yi(t), new Yi(e)).ComputeDiff(n).changes;
}
class at {
  static Assert(e, n) {
    if (!e)
...
class Ge {
  ...
  ComputeDiffRecursive(e, n, i, r, s) {
    ...
    if (e > n || i > r) {
      let d;
      return i <= r ? (at.Assert(e === n + 1, "originalStart should only be one more than originalEnd"), d = [
        new He(e, 0, i, r - i + 1)
      ]) : e <= n ? (at.Assert(i === r + 1, "modifiedStart should only be one more than modifiedEnd"), d = [
        new He(e, n - e + 1, i, 0)
      ]) : (at.Assert(e === n + 1, "originalStart should only be one more than originalEnd"), at.Assert(i === r + 1, "modifiedStart should only be one more than modifiedEnd"), d = []), d;
    }
    const a = [0], o = [0], l = this.ComputeRecursionPoint(e, n, i, r, a, o, s), c = a[0], h = o[0];
    if (l !== null)
      return l;
    if (!s[0]) {
      const d = this.ComputeDiffRecursive(e, c, i, h, s);
      let f = [];
      return s[0] ? f = [
        new He(c + 1, n - (c + 1) + 1, h + 1, r - (h + 1) + 1)
      ] : f = this.ComputeDiffRecursive(c + 1, n, h + 1, r, s), this.ConcatenateChanges(d, f);
    }
    return [
      new He(e, n - e + 1, i, r - i + 1)
    ];
  }
  WALKTRACE(e, n, i, r, s, a, o, l, c, h, d, f, m, b, D, E, L, F) {
    let v = null, C = null, x = new Zi(), w = n, N = i, B = m[0] - E[0] - r, j = -1073741824, ue = this.m_forwardHistory.length - 1;
    do {
      const G = B + e;
      G === w || G < N && c[G - 1] < c[G + 1] ? (d = c[G + 1], b = d - B - r, d < j && x.MarkNextChange(), j = d, x.AddModifiedElement(d + 1, b), B = G + 1 - e) : (d = c[G - 1] + 1, b = d - B - r, d < j && x.MarkNextChange(), j = d - 1, x.AddOriginalElement(d, b + 1), B = G - 1 - e), ue >= 0 && (c = this.m_forwardHistory[ue], e = c[0], w = 1, N = c.length - 1);
    } while (--ue >= -1);
    if (v = x.getReverseChanges(), F[0]) {
      let G = m[0] + 1, U = E[0] + 1;
      ...
      x = new Zi(), w = a, N = o, B = m[0] - E[0] - l, j = 1073741824, ue = L ? this.m_reverseHistory.length - 1 : this.m_reverseHistory.length - 2;
      do {
        const G = B + s;
        G === w || G < N && h[G - 1] >= h[G + 1] ? (d = h[G + 1] - 1, b = d - B - l, d > j && x.MarkNextChange(), j = d + 1, x.AddOriginalElement(d + 1, b + 1), B = G + 1 - s) : (d = h[G - 1], b = d - B - l, d > j && x.MarkNextChange(), j = d, x.AddModifiedElement(d + 1, b + 1), B = G - 1 - s), ue >= 0 && (h = this.m_reverseHistory[ue], s = h[0], w = 1, N = h.length - 1);
      } while (--ue >= -1);
      C = x.getChanges();
    }
  ...
  ComputeRecursionPoint(e, n, i, r, s, a, o) {
    let l = 0, c = 0, h = 0, d = 0, f = 0, m = 0;
    e--, i--, s[0] = 0, a[0] = 0, this.m_forwardHistory = [], this.m_reverseHistory = [];
    const b = n - e + (r - i), D = b + 1, E = new Int32Array(D), L = new Int32Array(D), F = r - i, v = n - e, C = e - i, x = n - r, N = (v - F) % 2 === 0;
    E[F] = e, L[v] = n, o[0] = !1;
    for (let B = 1; B <= b / 2 + 1; B++) {
      let j = 0, ue = 0;
      h = this.ClipDiagonalBound(F - B, B, F, D), d = this.ClipDiagonalBound(F + B, B, F, D);
      for (let U = h; U <= d; U += 2) {
        U === h || U < d && E[U - 1] < E[U + 1] ? l = E[U + 1] : l = E[U - 1] + 1, c = l - (U - F) - C;
        const y = l;
        for (; l < n && c < r && this.ElementsAreEqual(l + 1, c + 1); )
          l++, c++;
        if (E[U] = l, l + c > j + ue && (j = l, ue = c), !N && Math.abs(U - v) <= B - 1 && l >= L[U])
          return s[0] = l, a[0] = c, y <= L[U] && B <= 1448 ? this.WALKTRACE(F, h, d, C, v, f, m, x, E, L, l, n, s, c, r, a, N, o) : null;
      }
      const G = (j - e + (ue - i) - B) / 2;
      if (this.ContinueProcessingPredicate !== null && !this.ContinueProcessingPredicate(j, G))
        return o[0] = !0, s[0] = j, a[0] = ue, G > 0 && B <= 1448 ? this.WALKTRACE(F, h, d, C, v, f, m, x, E, L, l, n, s, c, r, a, N, o) : (e++, i++, [
          new He(e, n - e + 1, i, r - i + 1)
        ]);
      ...
      for (let U = f; U <= m; U += 2) {
        ...
        for (; l > e && c > i && this.ElementsAreEqual(l, c); )
          l--, c--;
        if (L[U] = l, N && Math.abs(U - F) <= B && l <= E[U])
          return s[0] = l, a[0] = c, y >= E[U] && B <= 1448 ? this.WALKTRACE(F, h, d, C, v, f, m, x, E, L, l, n, s, c, r, a, N, o) : null;
      }
      if (B <= 1447) {
        let U = new Int32Array(d - h + 2);
        U[0] = F - h + 1, ot.Copy2(E, h, U, 1, d - h + 1), this.m_forwardHistory.push(U), U = new Int32Array(m - f + 2), U[0] = v - f + 1, ot.Copy2(L, f, U, 1, m - f + 1), this.m_reverseHistory.push(U);
      }
    }
    return this.WALKTRACE(F, h, d, C, v, f, m, x, E, L, l, n, s, c, r, a, N, o);
  }
  /**
  ...
  PrettifyChanges(e) {
    ...
    for (let n = e.length - 1; n >= 0; n--) {
      ...
      for (let d = 1; ; d++) {
        const f = i.originalStart - d, m = i.modifiedStart - d;
    ...
    if (this._hasStrings)
      for (let n = 1, i = e.length; n < i; n++) {
        const r = e[n - 1], s = e[n], a = s.originalStart - r.originalStart - r.originalLength, o = r.originalStart, l = s.originalStart + s.originalLength, c = l - o, h = r.modifiedStart, d = s.modifiedStart + s.modifiedLength, f = d - h;
        if (a < 5 && c < 20 && f < 20) {
          const m = this._findBetterContiguousSequence(o, c, h, f, a);
          if (m) {
            const [b, D] = m;
            (b !== r.originalStart + r.originalLength || D !== r.modifiedStart + r.modifiedLength) && (r.originalLength = b - r.originalStart, r.modifiedLength = D - r.modifiedStart, s.originalStart = b + a, s.modifiedStart = D + a, s.originalLength = l - s.originalStart, s.modifiedLength = d - s.modifiedStart);
          }
        }
      }
    return e;
  ...
  _findBetterContiguousSequence(e, n, i, r, s) {
    ...
    for (let d = e; d < a; d++)
      for (let f = i; f < o; f++) {
        const m = this._contiguousSequenceScore(d, f, s);
        m > 0 && m > l && (l = m, c = d, h = f);
      }
    return l > 0 ? [c, h] : null;
  }
...
let ne = class Ze {
  ...
  static intersectRanges(e, n) {
    ...
    const o = n.startLineNumber, l = n.startColumn, c = n.endLineNumber, h = n.endColumn;
    return i < o ? (i = o, r = l) : i === o && (r = Math.max(r, l)), s > c ? (s = c, a = h) : s === c && (a = Math.min(a, h)), i > s || i === s && r > a ? null : new ie(i, r, s, a);
  }
  /**
    ...
    if (e && n) {
      ...
      if (s === a) {
        ...
        if (o === l) {
          ...
          if (c === h) {
            const d = e.endColumn | 0, f = n.endColumn | 0;
            return d - f;
          }
          return c - h;
        }
      ...
      for (; c < l; ) {
        ...
        if (f === 13) {
          ...
          switch (F) {
            ...
            case 34:
            case 96:
              d === F ? v = 1 : d === 39 || d === 34 || d === 96 ? v = 0 : v = 1;
              break;
            case 42:
              v = d === 42 ? 1 : 0;
              break;
          ...
          v === 1 && (r.push(ln._createLink(i, o, s, h, c)), L = !0);
        } else if (f === 12) {
          let v;
          F === 91 ? (b = !0, v = 0) : v = i.get(F), v === 1 ? L = !0 : f = 13;
        } else
          f = n.nextState(f, F), f === 0 && (L = !0);
        L && (f = 1, m = !1, b = !1, E = !1, h = c + 1, d = F), c++;
      }
      f === 13 && r.push(ln._createLink(i, o, s, h, l));
    }
    return r;
  }
}
function Yu(t) {
  return !t || typeof t.getLineCount != "function" || typeof t.getLineContent != "function" ? [] : ln.computeLinks(t);
}
class Ii {
  constructor() {
...
const eo = Object.freeze(function(t, e) {
...
(function(t) {
  function e(n) {
    return n === t.None || n === t.Cancelled || n instanceof jt ? !0 : !n || typeof n != "object" ? !1 : typeof n.isCancellationRequested == "boolean" && typeof n.onCancellationRequested == "function";
  }
...
class jt {
  ...
  get onCancellationRequested() {
    return this._isCancelled ? eo : (this._emitter || (this._emitter = new we()), this._emitter.event);
  }
  dispose() {
...
class Zu {
  ...
  cancel() {
    this._token ? this._token instanceof jt && this._token.cancel() : this._token = cn.Cancelled;
  }
  dispose(e = !1) {
    e && this.cancel(), this._parentListener?.dispose(), this._token ? this._token instanceof jt && this._token.dispose() : this._token = cn.None;
  }
}
class Ri {
  constructor() {
...
const Gt = new Ri(), Qn = new Ri(), Yn = new Ri(), Ku = new Array(230), el = /* @__PURE__ */ Object.create(null), tl = /* @__PURE__ */ Object.create(null);
(function() {
  const t = "", e = [
  ...
  for (const r of e) {
    const [s, a, o, l, c, h, d, f, m] = r;
    if (i[a] || (i[a] = !0, el[o] = a, tl[o.toLowerCase()] = a), !n[l]) {
      if (n[l] = !0, !c)
        throw new Error(`String representation missing for key code ${l} around scan code ${o}`);
      Gt.define(l, c), Qn.define(l, f || c), Yn.define(l, m || f || c);
    }
    h && (Ku[h] = l);
  }
})();
...
(function(t) {
  function e(o) {
    return Gt.keyCodeToStr(o);
  }
  t.toString = e;
  function n(o) {
    return Gt.strToKeyCode(o);
  }
  t.fromString = n;
  function i(o) {
...
class to {
  ...
  register(e, n) {
    return this._tokenizationSupports.set(e, n), this.handleChange([e]), tn(() => {
  ...
  registerFactory(e, n) {
    ...
    const i = new al(this, e, n);
    return this._factories.set(e, i), tn(() => {
...
class hn {
  ...
  static getChannel(e) {
    return e.getChannel(hn.CHANNEL_NAME);
  }
  static setChannel(e, n) {
    e.setChannel(hn.CHANNEL_NAME, n);
  }
}
var ns;
class cl {
...
function Dl(t, e, n, i, r) {
  return pl(t, e, n, i, r) && bl(t, e, n, i, r);
}
class xl {
  constructor(e, n) {
  ...
  next(e) {
    ...
    do {
      ...
      if (r === this._prevMatchStartIndex && s === this._prevMatchLength) {
        if (s === 0) {
          du(e, n, this._searchRegex.lastIndex) > 65535 ? this._searchRegex.lastIndex += 2 : this._searchRegex.lastIndex += 1;
          continue;
        }
        return null;
      }
      if (this._prevMatchStartIndex = r, this._prevMatchLength = s, !this._wordSeparators || Dl(this._wordSeparators, e, n, r, s))
        return i;
    } while (i);
    return null;
  }
}
function Cl(t, e = "Unreachable") {
  throw new Error(e);
}
function dn(t) {
  if (!t()) {
    debugger;
    t(), _t(new Ae("Assertion Failed"));
  }
}
function no(t, e) {
  let n = 0;
...
function Mi(t, e, n, i, r) {
  if (e = ro(e), r || (r = en.first(so)), n.length > r.maxLen) {
    let c = t - r.maxLen / 2;
    return c < 0 ? c = 0 : i += c, n = n.substring(c, t + r.maxLen / 2), Mi(t, e, n, i, r);
  }
...
class Ll {
  static computeUnicodeHighlights(e, n, i) {
    const r = i ? i.startLineNumber : 1, s = i ? i.endLineNumber : e.getLineCount(), a = new os(n), o = a.getCandidateCodePoints();
    let l;
    o === "allNonBasicAscii" ? l = new RegExp("[^\\t\\n\\r\\x20-\\x7E]", "g") : l = new RegExp(`${vl(Array.from(o))}`, "g");
    const c = new xl(null, l), h = [];
    let d = !1, f, m = 0, b = 0, D = 0;
    e: for (let E = r, L = s; E <= L; E++) {
      const F = e.getLineContent(E), v = F.length;
      c.reset(0);
      do
        if (f = c.next(F), f) {
          let C = f.index, x = f.index + f[0].length;
          if (C > 0) {
            const j = F.charCodeAt(C - 1);
            jn(j) && C--;
          }
          if (x + 1 < v) {
            const j = F.charCodeAt(x - 1);
            jn(j) && x++;
          }
          const w = F.substring(C, x);
          let N = Mi(C + 1, io, F, 0);
          N && N.endColumn <= C + 1 && (N = null);
          const B = a.shouldHighlightNonBasicASCII(w, N ? N.word : null);
          if (B !== 0) {
            if (B === 3 ? m++ : B === 2 ? b++ : B === 1 ? D++ : Cl(), h.length >= 1e3) {
              d = !0;
              break e;
            }
            h.push(new H(E, C + 1, E, x + 1));
          }
        }
      while (f);
    }
...
function vl(t, e) {
  return `[${au(t.map((i) => String.fromCodePoint(i)).join(""))}]`;
}
class os {
  constructor(e) {
  ...
  getCandidateCodePoints() {
    ...
    if (this.options.invisibleCharacters)
      for (const n of et.codePoints)
        us(String.fromCodePoint(n)) || e.add(n);
  ...
  shouldHighlightNonBasicASCII(e, n) {
    ...
    if (n)
      for (const a of n) {
        const o = a.codePointAt(0), l = mu(a);
    ...
    return (
      ...
      s ? 0 : this.options.invisibleCharacters && !us(e) && et.isInvisibleCharacter(i) ? 2 : this.options.ambiguousCharacters && this.ambiguousCharacters.isAmbiguous(i) ? 3 : 0
    );
  }
}
function us(t) {
...
function ni(t, e, n = 0, i = t.length) {
  let r = n, s = i;
  for (; r < s; ) {
    const a = Math.floor((r + s) / 2);
    e(t[a]) ? s = a : r = a + 1;
...
class Be {
  ...
  addRange(e) {
    if (e.length === 0)
      return;
    const n = ni(this._normalizedRanges, (r) => r.endLineNumberExclusive >= e.startLineNumber), i = Bt(this._normalizedRanges, (r) => r.startLineNumber <= e.endLineNumberExclusive) + 1;
    if (n === i)
   ...
   */
  subtractFrom(e) {
    const n = ni(this._normalizedRanges, (a) => a.endLineNumberExclusive >= e.startLineNumber), i = Bt(this._normalizedRanges, (a) => a.startLineNumber <= e.endLineNumberExclusive) + 1;
    if (n === i)
      return new Be([e]);
    const r = [];
  ...
  */
  toRangeMapping() {
    ...
    if (this.original.startLineNumber === 1 || this.modified.startLineNumber === 1) {
      ...
      return new _e(new H(this.original.startLineNumber, 1, this.original.endLineNumberExclusive, 1), new H(this.modified.startLineNumber, 1, this.modified.endLineNumberExclusive, 1));
    } else
      return new _e(new H(this.original.startLineNumber - 1, Number.MAX_SAFE_INTEGER, this.original.endLineNumberExclusive - 1, Number.MAX_SAFE_INTEGER), new H(this.modified.startLineNumber - 1, Number.MAX_SAFE_INTEGER, this.modified.endLineNumberExclusive - 1, Number.MAX_SAFE_INTEGER));
  }
  /**
   * This method assumes that the LineRangeMapping describes a valid diff!
   * I.e. if one range is empty, the other range cannot be the entire document.
  ...
  */
  toRangeMapping2(e, n) {
    ...
    if (!this.original.isEmpty && !this.modified.isEmpty)
      return new _e(H.fromPositions(new ne(this.original.startLineNumber, 1), lt(new ne(this.original.endLineNumberExclusive - 1, Number.MAX_SAFE_INTEGER), e)), H.fromPositions(new ne(this.modified.startLineNumber, 1), lt(new ne(this.modified.endLineNumberExclusive - 1, Number.MAX_SAFE_INTEGER), n)));
    if (this.original.startLineNumber > 1 && this.modified.startLineNumber > 1)
      return new _e(H.fromPositions(lt(new ne(this.original.startLineNumber - 1, Number.MAX_SAFE_INTEGER), e), lt(new ne(this.original.endLineNumberExclusive - 1, Number.MAX_SAFE_INTEGER), e)), H.fromPositions(lt(new ne(this.modified.startLineNumber - 1, Number.MAX_SAFE_INTEGER), n), lt(new ne(this.modified.endLineNumberExclusive - 1, Number.MAX_SAFE_INTEGER), n)));
    throw new Ae();
  }
}
function lt(t, e) {
...
class Nl {
  computeDiff(e, n, i) {
    ...
    for (const l of s.changes) {
      ...
      let h;
      l.modifiedEndLineNumber === 0 ? h = new $(l.modifiedStartLineNumber + 1, l.modifiedStartLineNumber + 1) : h = new $(l.modifiedStartLineNumber, l.modifiedEndLineNumber + 1);
      let d = new Te(c, h, l.charChanges?.map((f) => new _e(new H(f.originalStartLineNumber, f.originalStartColumn, f.originalEndLineNumber, f.originalEndColumn), new H(f.modifiedStartLineNumber, f.modifiedStartColumn, f.modifiedEndLineNumber, f.modifiedEndColumn))));
      o && (o.modified.endLineNumberExclusive === d.modified.startLineNumber || o.original.endLineNumberExclusive === d.original.startLineNumber) && (d = new Te(o.original.join(d.original), o.modified.join(d.modified), o.innerChanges && d.innerChanges ? o.innerChanges.concat(d.innerChanges) : void 0), a.pop()), a.push(d), o = d;
    }
    return dn(() => no(a, (l, c) => c.original.startLineNumber - l.original.endLineNumberExclusive === c.modified.startLineNumber - l.modified.endLineNumberExclusive && // There has to be an unchanged line in between (otherwise both diffs should have been joined)
    l.original.endLineNumberExclusive < c.original.startLineNumber && l.modified.endLineNumberExclusive < c.modified.startLineNumber)), new Xt(a, [], s.quitEarly);
  }
}
function ao(t, e, n, i) {
...
function Bl(t) {
  ...
  for (let i = 1, r = t.length; i < r; i++) {
    ...
    Math.min(a, o) < Sl ? (n.originalLength = s.originalStart + s.originalLength - n.originalStart, n.modifiedLength = s.modifiedStart + s.modifiedLength - n.modifiedStart) : (e.push(s), n = s);
  }
  return e;
}
class yt {
  ...
  static createFromDiffResult(e, n, i, r, s, a, o) {
    let l, c, h, d, f;
    if (n.originalLength === 0 ? (l = i.getStartLineNumber(n.originalStart) - 1, c = 0) : (l = i.getStartLineNumber(n.originalStart), c = i.getEndLineNumber(n.originalStart + n.originalLength - 1)), n.modifiedLength === 0 ? (h = r.getStartLineNumber(n.modifiedStart) - 1, d = 0) : (h = r.getStartLineNumber(n.modifiedStart), d = r.getEndLineNumber(n.modifiedStart + n.modifiedLength - 1)), a && n.originalLength > 0 && n.originalLength < 20 && n.modifiedLength > 0 && n.modifiedLength < 20 && s()) {
...
class Il {
  ...
  computeDiff() {
    ...
    const e = ao(this.original, this.modified, this.continueLineDiff, this.shouldMakePrettyDiff), n = e.changes, i = e.quitEarly;
    if (this.shouldIgnoreTrimWhitespace) {
      const o = [];
      for (let l = 0, c = n.length; l < c; l++)
        o.push(yt.createFromDiffResult(this.shouldIgnoreTrimWhitespace, n[l], this.original, this.modified, this.continueCharDiff, this.shouldComputeCharChanges, this.shouldPostProcessCharChanges));
      return {
        quitEarly: i,
        changes: o
      };
    ...
    for (let o = -1, l = n.length; o < l; o++) {
      const c = o + 1 < l ? n[o + 1] : null, h = c ? c.originalStart : this.originalLines.length, d = c ? c.modifiedStart : this.modifiedLines.length;
      for (; s < h && a < d; ) {
        const f = this.originalLines[s], m = this.modifiedLines[a];
        if (f !== m) {
          {
            let b = ii(f, 1), D = ii(m, 1);
            for (; b > 1 && D > 1; ) {
          ...
          {
            let b = ri(f, 1), D = ri(m, 1);
            const E = f.length + 1, L = m.length + 1;
            for (; b < E && D < L; ) {
              const F = f.charCodeAt(b - 1), v = f.charCodeAt(D - 1);
              if (F !== v)
                break;
              b++, D++;
            }
      ...
      c && (r.push(yt.createFromDiffResult(this.shouldIgnoreTrimWhitespace, c, this.original, this.modified, this.continueCharDiff, this.shouldComputeCharChanges, this.shouldPostProcessCharChanges)), s += c.originalLength, a += c.modifiedLength);
    }
    return {
      quitEarly: i,
      changes: r
  ...
  _pushTrimWhitespaceCharChange(e, n, i, r, s, a, o) {
    if (this._mergeTrimWhitespaceCharChange(e, n, i, r, s, a, o))
      return;
    let l;
    this.shouldComputeCharChanges && (l = [new pt(n, i, n, r, s, a, s, o)]), e.push(new yt(n, n, s, s, l));
  }
...
class Yt {
  ...
  filter(e) {
    return new Yt((n) => this.iterate((i) => e(i) ? n(i) : !0));
  }
  map(e) {
    return new Yt((n) => this.iterate((i) => n(e(i))));
  }
  findLast(e) {
    let n;
...
class ql {
  compute(e, n, i = Ut.instance, r) {
    if (e.length === 0 || n.length === 0)
      return Ue.trivial(e, n);
    const s = new Pn(e.length, n.length), a = new Pn(e.length, n.length), o = new Pn(e.length, n.length);
    for (let b = 0; b < e.length; b++)
      for (let D = 0; D < n.length; D++) {
        ...
        let F;
        e.getElement(b) === n.getElement(D) ? (b === 0 || D === 0 ? F = 0 : F = s.get(b - 1, D - 1), b > 0 && D > 0 && a.get(b - 1, D - 1) === 3 && (F += o.get(b - 1, D - 1)), F += r ? r(b, D) : 1) : F = -1;
        const v = Math.max(E, L, F);
        if (v === F) {
    ...
    function d(b, D) {
      (b + 1 !== c || D + 1 !== h) && l.push(new K(new O(b + 1, c), new O(D + 1, h))), c = b, h = D;
    }
    let f = e.length - 1, m = n.length - 1;
    for (; f >= 0 && m >= 0; )
      a.get(f, m) === 3 ? (d(f, m), f--, m--) : a.get(f, m) === 1 ? f-- : m--;
    return d(-1, -1), l.reverse(), new Ue(l, !1);
  }
}
class oo {
  compute(e, n, i = Ut.instance) {
    if (e.length === 0 || n.length === 0)
      return Ue.trivial(e, n);
    const r = e, s = n;
    function a(D, E) {
    ...
    l.set(0, a(0, 0));
    const c = new Wl();
    c.set(0, l.get(0) === 0 ? null : new ds(null, 0, 0, l.get(0)));
    let h = 0;
    e: for (; ; ) {
      if (o++, !i.isValid())
      ...
      for (h = D; h <= E; h += 2) {
        const L = h === E ? -1 : l.get(h + 1), F = h === D ? -1 : l.get(h - 1) + 1, v = Math.min(Math.max(L, F), r.length), C = v - h;
        if (v > r.length || C > s.length)
          continue;
        const x = a(v, C);
        l.set(h, x);
        const w = v === L ? c.get(h + 1) : c.get(h - 1);
        if (c.set(h, x !== v ? new ds(w, v, C, x - v) : w), l.get(h) === r.length && l.get(h) - h === s.length)
          break e;
      }
    }
    let d = c.get(h);
    ...
    for (; ; ) {
      const D = d ? d.x + d.length : 0, E = d ? d.y + d.length : 0;
      if ((D !== m || E !== b) && f.push(new K(new O(D, m), new O(E, b))), !d)
        break;
      m = d.x, b = d.y, d = d.prev;
    }
...
class fn {
  ...
  getBoundaryScore(e) {
    const n = ms(e > 0 ? this.elements[e - 1] : -1), i = ms(e < this.elements.length ? this.elements[e] : -1);
    if (n === 7 && i === 8)
      return 0;
    if (n === 8)
      return 150;
    let r = 0;
    return n !== i && (r += 10, n === 0 && i === 1 && (r += 1)), r += fs(n), r += fs(i), r;
  }
  translateOffset(e, n = "right") {
    const i = Bt(this.firstElementOffsetByLineIdx, (s) => s <= e), r = e - this.firstElementOffsetByLineIdx[i];
  ...
  findWordContaining(e) {
    if (e < 0 || e >= this.elements.length || !Tn(this.elements[e]))
      return;
    let n = e;
    for (; n > 0 && Tn(this.elements[n - 1]); )
      n--;
    let i = e;
    for (; i < this.elements.length && Tn(this.elements[i]); )
      i++;
    return new O(n, i);
  }
  countLinesIn(e) {
  ...
  extendToFullLines(e) {
    const n = Dt(this.firstElementOffsetByLineIdx, (r) => r <= e.start) ?? 0, i = _l(this.firstElementOffsetByLineIdx, (r) => e.endExclusive <= r) ?? this.elements.length;
    return new O(n, i);
  }
}
function Tn(t) {
...
function ms(t) {
  return t === 10 ? 8 : t === 13 ? 7 : ai(t) ? 6 : t >= 97 && t <= 122 ? 0 : t >= 65 && t <= 90 ? 1 : t >= 48 && t <= 57 ? 2 : t === -1 ? 3 : t === 44 || t === 59 ? 5 : 4;
}
function Hl(t, e, n, i, r, s) {
  let { moves: a, excludedChanges: o } = Gl(t, e, n, s);
  if (!s.isValid())
    return [];
  const l = t.filter((h) => !o.has(h)), c = Xl(l, i, r, e, n, s);
  return Ul(a, c), a = Jl(a), a = a.filter((h) => {
    const d = h.original.toOffsetRange().slice(e).map((m) => m.trim());
    return d.join(`
`).length >= 15 && jl(d, (m) => m.length >= 2) >= 2;
  }), a = Ql(t, a), a;
}
function jl(t, e) {
  let n = 0;
...
function Gl(t, e, n, i) {
  const r = [], s = t.filter((l) => l.modified.isEmpty && l.original.length >= 3).map((l) => new It(l.original, e, l)), a = new Set(t.filter((l) => l.original.isEmpty && l.modified.length >= 3).map((l) => new It(l.modified, n, l))), o = /* @__PURE__ */ new Set();
  for (const l of s) {
    let c = -1, h;
    for (const d of a) {
      const f = l.computeSimilarity(d);
      f > c && (c = f, h = d);
    }
    if (c > 0.9 && h && (a.delete(h), r.push(new ve(l.range, h.range)), o.add(l.source), o.add(h.source)), !i.isValid())
...
function Xl(t, e, n, i, r, s) {
  ...
  const l = [];
  t.sort(Jt((f) => f.modified.startLineNumber, Qt));
  for (const f of t) {
    let m = [];
    for (let b = f.modified.startLineNumber; b < f.modified.endLineNumberExclusive - 2; b++) {
      ...
      o.forEach(D, ({ range: F }) => {
        for (const C of m)
          if (C.originalLineRange.endLineNumberExclusive + 1 === F.endLineNumberExclusive && C.modifiedLineRange.endLineNumberExclusive + 1 === E.endLineNumberExclusive) {
            C.originalLineRange = new $(C.originalLineRange.startLineNumber, F.endLineNumberExclusive), C.modifiedLineRange = new $(C.modifiedLineRange.startLineNumber, E.endLineNumberExclusive), L.push(C);
            return;
          }
        const v = {
          modifiedLineRange: E,
  ...
  l.sort(Vl(Jt((f) => f.modifiedLineRange.length, Qt)));
  const c = new Be(), h = new Be();
  for (const f of l) {
    const m = f.modifiedLineRange.startLineNumber - f.originalLineRange.startLineNumber, b = c.subtractFrom(f.modifiedLineRange), D = h.subtractFrom(f.originalLineRange).getWithDelta(m), E = b.getIntersection(D);
    for (const L of E.ranges) {
      if (L.length < 3)
        continue;
      const F = L, v = L.delta(-m);
      a.push(new ve(v, F)), c.addRange(F), h.addRange(v);
    }
  }
  a.sort(Jt((f) => f.original.startLineNumber, Qt));
  const d = new yn(t);
  for (let f = 0; f < a.length; f++) {
    const m = a[f], b = d.findLastMonotonous((w) => w.original.startLineNumber <= m.original.startLineNumber), D = Dt(t, (w) => w.modified.startLineNumber <= m.modified.startLineNumber), E = Math.max(m.original.startLineNumber - b.original.startLineNumber, m.modified.startLineNumber - D.modified.startLineNumber), L = d.findLastMonotonous((w) => w.original.startLineNumber < m.original.endLineNumberExclusive), F = Dt(t, (w) => w.modified.startLineNumber < m.modified.endLineNumberExclusive), v = Math.max(L.original.endLineNumberExclusive - m.original.endLineNumberExclusive, F.modified.endLineNumberExclusive - m.modified.endLineNumberExclusive);
    let C;
    for (C = 0; C < E; C++) {
      const w = m.original.startLineNumber - C - 1, N = m.modified.startLineNumber - C - 1;
      if (w > i.length || N > r.length || c.contains(N) || h.contains(w) || !gs(i[w - 1], r[N - 1], s))
        break;
    }
    C > 0 && (h.addRange(new $(m.original.startLineNumber - C, m.original.startLineNumber)), c.addRange(new $(m.modified.startLineNumber - C, m.modified.startLineNumber)));
    let x;
    for (x = 0; x < v; x++) {
    ...
    x > 0 && (h.addRange(new $(m.original.endLineNumberExclusive, m.original.endLineNumberExclusive + x)), c.addRange(new $(m.modified.endLineNumberExclusive, m.modified.endLineNumberExclusive + x))), (C > 0 || x > 0) && (a[f] = new ve(new $(m.original.startLineNumber - C, m.original.endLineNumberExclusive + x), new $(m.modified.startLineNumber - C, m.modified.endLineNumberExclusive + x)));
  }
  return a;
}
function gs(t, e, n) {
  ...
  if (t.length > 300 && e.length > 300)
    return !1;
  const r = new oo().compute(new fn([t], new H(1, 1, 1, t.length), !1), new fn([e], new H(1, 1, 1, e.length), !1), n);
  let s = 0;
...
function Jl(t) {
  if (t.length === 0)
    return t;
  t.sort(Jt((n) => n.original.startLineNumber, Qt));
  const e = [t[0]];
...
function ps(t, e, n) {
  let i = n;
  return i = bs(t, e, i), i = bs(t, e, i), i = Yl(t, e, i), i;
}
function bs(t, e, n) {
  if (n.length === 0)
    return n;
...
function Yl(t, e, n) {
  ...
  for (let i = 0; i < n.length; i++) {
    ...
    s.seq1Range.isEmpty ? n[i] = Ds(s, t, e, o, l) : s.seq2Range.isEmpty && (n[i] = Ds(s.swap(), e, t, l, o).swap());
  }
  return n;
}
function Ds(t, e, n, i, r) {
  ...
  let l = 0, c = -1;
  for (let h = -a; h <= o; h++) {
    const d = t.seq2Range.start + h, f = t.seq2Range.endExclusive + h, m = t.seq1Range.start + h, b = e.getBoundaryScore(m) + n.getBoundaryScore(d) + n.getBoundaryScore(f);
    b > c && (c = b, l = h);
  }
  return t.delta(l);
}
...
function Kl(t, e, n) {
  ...
  function a(l, c) {
    if (l.offset1 < s.offset1 || l.offset2 < s.offset2)
      return;
    const h = t.findWordContaining(l.offset1), d = e.findWordContaining(l.offset2);
    if (!h || !d)
      return;
    let f = new K(h, d);
    ...
    for (; i.length > 0; ) {
      ...
      if (!(E.seq1Range.intersects(f.seq1Range) || E.seq2Range.intersects(f.seq2Range)))
        break;
      const F = t.findWordContaining(E.seq1Range.start), v = e.findWordContaining(E.seq2Range.start), C = new K(F, v), x = C.intersect(E);
...
function tc(t, e, n) {
  ...
  do {
    ...
    for (let l = 1; l < i.length; l++) {
      let d = function(m, b) {
        const D = new O(h.seq1Range.endExclusive, c.seq1Range.start);
        return t.getText(D).replace(/\s/g, "").length <= 4 && (m.seq1Range.length + m.seq2Range.length > 5 || b.seq1Range.length + b.seq2Range.length > 5);
      };
      var a = d;
      const c = i[l], h = o[o.length - 1];
      d(h, c) ? (s = !0, o[o.length - 1] = o[o.length - 1].join(c)) : o.push(c);
    }
    i = o;
  } while (r++ < 10 && s);
  return i;
...
function nc(t, e, n) {
  ...
  do {
    ...
    for (let c = 1; c < i.length; c++) {
      let f = function(b, D) {
        const E = new O(d.seq1Range.endExclusive, h.seq1Range.start);
        if (t.countLinesIn(E) > 5 || E.length > 500)
          return !1;
        const F = t.getText(E).trim();
      ...
      var o = f;
      const h = i[c], d = l[l.length - 1];
      f(d, h) ? (s = !0, l[l.length - 1] = l[l.length - 1].join(h)) : l.push(h);
    }
    i = l;
  } while (r++ < 10 && s);
  const a = [];
  return Tl(i, (l, c, h) => {
    let d = c;
    function f(F) {
      return F.length > 0 && F.trim().length <= 3 && c.seq1Range.length + c.seq2Range.length > 100;
    }
    ...
    f(D) && (d = d.deltaEnd(D.length));
    const E = K.fromOffsetPairs(l ? l.getEndExclusives() : Se.zero, h ? h.getStarts() : Se.max), L = d.intersect(E);
...
class xs {
  ...
  getBoundaryScore(e) {
    const n = e === 0 ? 0 : Cs(this.lines[e - 1]), i = e === this.lines.length ? 0 : Cs(this.lines[e]);
    return 1e3 - (n + i);
  }
  getText(e) {
...
class ic {
  constructor() {
    this.dynamicProgrammingDiffing = new ql(), this.myersDiffingAlgorithm = new oo();
  }
  computeDiff(e, n, i) {
    if (e.length <= 1 && Rl(e, n, (x, w) => x === w))
      return new Xt([], [], !1);
    if (e.length === 1 && e[0].length === 0 || n.length === 1 && n[0].length === 0)
      return new Xt([
        new Te(new $(1, e.length + 1), new $(1, n.length + 1), [
          new _e(new H(1, 1, e.length, e[e.length - 1].length + 1), new H(1, 1, n.length, n[n.length - 1].length + 1))
        ])
      ], [], !1);
    const r = i.maxComputationTimeMs === 0 ? Ut.instance : new $l(i.maxComputationTimeMs), s = !i.ignoreTrimWhitespace, a = /* @__PURE__ */ new Map();
    function o(x) {
      let w = a.get(x);
      return w === void 0 && (w = a.size, a.set(x, w)), w;
    }
    const l = e.map((x) => o(x.trim())), c = n.map((x) => o(x.trim())), h = new xs(l, e), d = new xs(c, n), f = h.length + d.length < 1700 ? this.dynamicProgrammingDiffing.compute(h, d, r, (x, w) => e[x] === n[w] ? n[w].length === 0 ? 0.1 : 1 + Math.log(1 + n[w].length) : 0.99) : this.myersDiffingAlgorithm.compute(h, d, r);
    let m = f.diffs, b = f.hitTimeout;
    m = ps(h, d, m), m = tc(h, d, m);
    const D = [], E = (x) => {
      if (s)
        for (let w = 0; w < x; w++) {
          const N = L + w, B = F + w;
    ...
    let L = 0, F = 0;
    for (const x of m) {
      dn(() => x.seq1Range.start - L === x.seq2Range.start - F);
      const w = x.seq1Range.start - L;
      E(w), L = x.seq1Range.endExclusive, F = x.seq2Range.endExclusive;
    ...
    const v = Es(D, e, n);
    let C = [];
    return i.computeMoves && (C = this.computeMoves(v, e, n, l, c, r, s)), dn(() => {
      function x(N, B) {
  ...
  computeMoves(e, n, i, r, s, a, o) {
    return Hl(e, n, i, r, s, a).map((h) => {
      const d = this.refineDiff(n, i, new K(h.original.toOffsetRange(), h.modified.toOffsetRange()), a, o), f = Es(d.mappings, n, i, !0);
      return new Fl(h, f);
    });
  }
  refineDiff(e, n, i, r, s) {
    const o = sc(i).toRangeMapping2(e, n), l = new fn(e, o.originalRange, s), c = new fn(n, o.modifiedRange, s), h = l.length + c.length < 500 ? this.dynamicProgrammingDiffing.compute(l, c, r) : this.myersDiffingAlgorithm.compute(l, c, r);
    let d = h.diffs;
    return d = ps(l, c, d), d = Kl(l, c, d), d = Zl(l, c, d), d = nc(l, c, d), {
      mappings: d.map((m) => new _e(l.translateRange(m.seq1Range), c.translateRange(m.seq2Range))),
      hitTimeout: h.hitTimeout
    };
  }
}
function Es(t, e, n, i = !1) {
  const r = [];
  for (const s of Ml(t.map((a) => rc(a, e, n)), (a, o) => a.original.overlapOrTouch(o.original) || a.modified.overlapOrTouch(o.modified))) {
    const a = s[0], o = s[s.length - 1];
    r.push(new Te(a.original.join(o.original), a.modified.join(o.modified), s.map((l) => l.innerChanges[0])));
  }
  return dn(() => !i && r.length > 0 && (r[0].modified.startLineNumber !== r[0].original.startLineNumber || n.length - r[r.length - 1].modified.endLineNumberExclusive !== e.length - r[r.length - 1].original.endLineNumberExclusive) ? !1 : no(r, (s, a) => a.original.startLineNumber - s.original.endLineNumberExclusive === a.modified.startLineNumber - s.modified.endLineNumberExclusive && // There has to be an unchanged line in between (otherwise both diffs should have been joined)
...
class Le {
  ...
  static fromRGBA(e) {
    ...
    const h = (o + a) / 2, d = a - o;
    if (d > 0) {
      switch (c = Math.min(h <= 0.5 ? d / (2 * h) : d / (2 - 2 * h), 1), a) {
        case n:
          l = (i - r) / d + (i < r ? 6 : 0);
          break;
        case i:
  ...
  static toRGBA(e) {
    ...
    if (i === 0)
      ...
      a = Le._hue2rgb(h, c, n + 1 / 3), o = Le._hue2rgb(h, c, n), l = Le._hue2rgb(h, c, n - 1 / 3);
    }
...
class mt {
  ...
  static toRGBA(e) {
    ...
    let [c, h, d] = [0, 0, 0];
    return n < 60 ? (c = a, h = o) : n < 120 ? (c = o, h = a) : n < 180 ? (h = a, d = o) : n < 240 ? (h = o, d = a) : n < 300 ? (c = o, d = a) : n <= 360 && (c = a, d = o), c = Math.round((c + l) * 255), h = Math.round((h + l) * 255), d = Math.round((d + l) * 255), new oe(c, h, d, s);
  }
}
let mn = class ae {
  static fromHex(e) {
  ...
  (function(e) {
    (function(n) {
      ...
      function d(m) {
        ...
        if (b === 9) {
          const D = 16 * f(m.charCodeAt(1)) + f(m.charCodeAt(2)), E = 16 * f(m.charCodeAt(3)) + f(m.charCodeAt(4)), L = 16 * f(m.charCodeAt(5)) + f(m.charCodeAt(6)), F = 16 * f(m.charCodeAt(7)) + f(m.charCodeAt(8));
          return new t(new oe(D, E, L, F / 255));
        }
        if (b === 4) {
        ...
        if (b === 5) {
          const D = f(m.charCodeAt(1)), E = f(m.charCodeAt(2)), L = f(m.charCodeAt(3)), F = f(m.charCodeAt(4));
          return new t(new oe(16 * D + D, 16 * E + E, 16 * L + L, (16 * F + F) / 255));
        }
        return null;
      }
      n.parseHex = d;
...
function ac(t, e) {
  if (!t)
    return;
  const n = mn.Format.CSS.parseHex(e);
  if (n)
    return {
      range: t,
...
function Ls(t, e, n) {
  if (!t || e.length !== 1)
    return;
  const r = e[0].values(), s = uo(r), a = new mn(new Le(s[0], s[1] / 100, s[2] / 100, n ? s[3] : 1));
  return {
    range: t,
...
function lo(t) {
  ...
  return t = t.replace(lc, ""), { text: t, hasSeparatorLine: e };
}
var Fs;
(function(t) {
  async function e(i) {
    ...
    return {
      next: async () => {
        do {
          ...
          await rn.toPromise(this._onStateChanged.event);
        } while (!0);
      },
...
class bc {
  ...
  $acceptNewModel(e) {
    this._models[e.url] = new Dc(ke.parse(e.url), e.lines, e.EOL, e.versionId);
  }
...
class Dc extends pc {
  ...
  getWordAtPosition(e, n) {
    const i = Mi(e.column, ro(n), this._lines[e.lineNumber - 1], 0);
...
class xc {
  constructor() {
    this._workerTextModelSyncServer = new bc();
  }
  dispose() {
  }
  _getModel(e) {
  ...
  async $computeUnicodeHighlights(e, n, i) {
    ...
    return r ? Ll.computeUnicodeHighlights(r, n, i) : { ranges: [], hasMore: !1, ambiguousCharacterCount: 0, invisibleCharacterCount: 0, nonBasicAsciiCharacterCount: 0 };
  }
  ...
  async $computeDiff(e, n, i, r) {
    ...
    return !s || !a ? null : Zt.computeDiff(s, a, i, r);
  }
  static computeDiff(e, n, i, r) {
    const s = r === "advanced" ? As.getDefault() : As.getLegacy(), a = e.getLinesContent(), o = n.getLinesContent(), l = s.computeDiff(a, o, i), c = l.changes.length > 0 ? !1 : this._modelsAreIdentical(e, n);
    function h(d) {
      return d.map((f) => [f.original.startLineNumber, f.original.endLineNumberExclusive, f.modified.startLineNumber, f.modified.endLineNumberExclusive, f.innerChanges?.map((m) => [
  ...
  async $computeMoreMinimalEdits(e, n, i) {
    ...
    for (let { range: l, text: c, eol: h } of n) {
      if (typeof h == "number" && (a = h), H.isEmpty(l) && !c)
        continue;
      const d = r.getValueInRange(l);
      if (c = c.replace(/\r\n|\n|\r/g, r.eol), d === c)
        continue;
      if (Math.max(c.length, d.length) > Zt._diffLimit) {
        s.push({ range: l, text: c });
        continue;
      }
      const f = ju(d, c, i), m = r.offsetAt(H.lift(l).getStartPosition());
      for (const b of f) {
  ...
  async $textualSuggest(e, n, i, r) {
    const s = new _n(), a = new RegExp(i, r), o = /* @__PURE__ */ new Set();
    e: for (const l of e) {
      const c = this._getModel(l);
      if (c) {
        for (const h of c.words(a))
          if (!(h === n || !isNaN(Number(h))) && (o.add(h), o.size > Zt._suggestionsLimit))
            break e;
      }
    }
  ...
  async $navigateValueSet(e, n, i, r, s) {
    ...
    return Ii.INSTANCE.navigateValueSet(n, l, c, h, i);
  }
}
class Zt extends xc {
  constructor(e, n) {
  ...
  $loadForeignModule(e, n, i) {
    const a = {
      host: gl(i, (o, l) => this._host.$fhr(o, l)),
      getMirrorModels: () => this._getModels()
    };
    return this._foreignModuleFactory ? (this._foreignModule = this._foreignModuleFactory(a, n), Promise.resolve(is(this._foreignModule))) : new Promise((o, l) => {
      const c = (h) => {
        this._foreignModule = h.create(a, n), o(is(this._foreignModule));
      };
      import(`${Ya.asBrowserUri(`${e}.js`).toString(!0)}`).then(c).catch(l);
    });
  }
...
typeof importScripts == "function" && (globalThis.monaco = ll());
let oi = !1;
function co(t) {
  if (oi)
    return;
...
(function(t) {
  function e(i, r) {
    return i === Number.MAX_VALUE && (i = gn.MAX_VALUE), r === Number.MAX_VALUE && (r = gn.MAX_VALUE), { line: i, character: r };
  }
  t.create = e;
  function n(i) {
    let r = i;
...
(function(t) {
  ...
  function n(i) {
    const r = i;
    return p.objectLiteral(r) && se.is(r.range) && li.is(r.color);
  }
  t.is = n;
})(Ns || (Ns = {}));
var ks;
...
(function(t) {
  ...
  function n(i) {
    let r = i;
    return p.defined(r) && pn.is(r.location) && p.string(r.message);
  }
  t.is = n;
})(ci || (ci = {}));
var ce;
...
(function(t) {
  ...
  function n(i) {
    var r;
    let s = i;
    return p.defined(s) && se.is(s.range) && p.string(s.message) && (p.number(s.severity) || p.undefined(s.severity)) && (p.integer(s.code) || p.string(s.code) || p.undefined(s.code)) && (p.undefined(s.codeDescription) || p.string((r = s.codeDescription) === null || r === void 0 ? void 0 : r.href)) && (p.string(s.source) || p.undefined(s.source)) && (p.undefined(s.relatedInformation) || p.typedArray(s.relatedInformation, ci.is));
  }
  t.is = n;
})(bn || (bn = {}));
var xt;
...
(function(t) {
  ...
  function r(s) {
    const a = s;
    return Ct.is(a) && (hi.is(a.annotationId) || Et.is(a.annotationId));
  }
  t.is = r;
})(Ps || (Ps = {}));
var di;
(function(t) {
  ...
  function n(i) {
    let r = i;
    return p.defined(r) && bi.is(r.textDocument) && Array.isArray(r.edits);
  }
  t.is = n;
})(di || (di = {}));
var fi;
...
(function(t) {
  function e(n) {
    let i = n;
    return i && (i.changes !== void 0 || i.documentChanges !== void 0) && (i.documentChanges === void 0 || i.documentChanges.every((r) => p.string(r.kind) ? fi.is(r) || mi.is(r) || gi.is(r) : di.is(r)));
  }
  t.is = e;
})(pi || (pi = {}));
var Ts;
...
(function(t) {
  function e(n) {
    const i = n;
    return p.objectLiteral(n) && Dn.is(i.kind) && p.string(i.value);
  }
  t.is = e;
})(Rt || (Rt = {}));
var re;
...
(function(t) {
  function e(n) {
    let i = n;
    return !!i && p.objectLiteral(i) && (Rt.is(i.contents) || xn.is(i.contents) || p.typedArray(i.contents, xn.is)) && (n.range === void 0 || se.is(n.range));
  }
  t.is = e;
})(Hs || (Hs = {}));
var js;
...
(function(t) {
  ...
  function n(i) {
    let r = i;
    return p.defined(r) && p.typedArray(r.diagnostics, bn.is) && (r.only === void 0 || p.typedArray(r.only, p.string)) && (r.triggerKind === void 0 || r.triggerKind === Cn.Invoked || r.triggerKind === Cn.Automatic);
  }
  t.is = n;
})(na || (na = {}));
var ia;
(function(t) {
  ...
  function n(i) {
    let r = i;
    return r && p.string(r.title) && (r.diagnostics === void 0 || p.typedArray(r.diagnostics, bn.is)) && (r.kind === void 0 || p.string(r.kind)) && (r.edit !== void 0 || r.command !== void 0) && (r.command === void 0 || xt.is(r.command)) && (r.isPreferred === void 0 || p.boolean(r.isPreferred)) && (r.edit === void 0 || pi.is(r.edit));
  }
  t.is = n;
})(ia || (ia = {}));
var ra;
...
(function(t) {
  ...
  function n(i) {
    const r = i;
    return p.objectLiteral(r) && (r.tooltip === void 0 || p.string(r.tooltip) || Rt.is(r.tooltip)) && (r.location === void 0 || pn.is(r.location)) && (r.command === void 0 || xt.is(r.command));
  }
  t.is = n;
})(xi || (xi = {}));
var ga;
(function(t) {
  ...
  function n(i) {
    const r = i;
    return p.objectLiteral(r) && ye.is(r.position) && (p.string(r.label) || p.typedArray(r.label, xi.is)) && (r.kind === void 0 || Di.is(r.kind)) && r.textEdits === void 0 || p.typedArray(r.textEdits, Ct.is) && (r.tooltip === void 0 || p.string(r.tooltip) || Rt.is(r.tooltip)) && (r.paddingLeft === void 0 || p.boolean(r.paddingLeft)) && (r.paddingRight === void 0 || p.boolean(r.paddingRight));
  }
  t.is = n;
})(ga || (ga = {}));
var pa;
...
(function(t) {
  function e(n) {
    const i = n;
    return p.objectLiteral(i) && ui.is(i.uri) && p.string(i.name);
  }
  t.is = e;
})(Aa || (Aa = {}));
var wa;
(function(t) {
  ...
  t.is = n;
  function i(s, a) {
    let o = s.getText(), l = r(a, (h, d) => {
      let f = h.range.start.line - d.range.start.line;
      return f === 0 ? h.range.start.character - d.range.start.character : f;
    }), c = o.length;
    for (let h = l.length - 1; h >= 0; h--) {
      let d = l[h], f = s.offsetAt(d.range.start), m = s.offsetAt(d.range.end);
      if (m <= c)
        o = o.substring(0, f) + d.newText + o.substring(m, o.length);
      else
  ...
  function r(s, a) {
    ...
    r(l, a), r(c, a);
    let h = 0, d = 0, f = 0;
    for (; h < l.length && d < c.length; )
      a(l[h], c[d]) <= 0 ? s[f++] = l[h++] : s[f++] = c[d++];
    for (; h < l.length; )
      s[f++] = l[h++];
    for (; d < c.length; )
      s[f++] = c[d++];
...
async function Ec(t, e) {
  try {
    const n = await t, i = /* @__PURE__ */ new Map(), r = /* @__PURE__ */ new Map(), s = [], a = Ac(e);
    return n.forEach((o) => {
      const l = [];
      o.items.forEach(
        (c) => wc(l, r, i, a, c)
...
function Ac(t) {
  const { profile: { apiVersion: e, bundles: n, hiddenApiItems: i = [] } = {} } = t, r = ho(e), s = Array.isArray(n) && n.length > 0 ? /* @__PURE__ */ new Set(["core", ...n]) : void 0;
  return (a) => vc(r, a.sinceVersion) && (!s || s.has(a.bundle)) && !i.includes(a.name);
}
function wc(t, e, n, i, r) {
  ...
  r.type === "constant" ? Lc(t, e, i, r) : La(t, n, i, r);
}
function La(t, e, n, i) {
  if (!n(i))
    return;
...
function Lc(t, e, n, i) {
  ...
  h.name = o, h.completion.label = o, h.completion.insertText = o, h.completion.kind = re.Constant, c.members.push(h), c.completion.documentation = c.members.reduce(
    (d, f) => (d.value = `${d.value}${d.value ? `

` : ""}**${f.completion.label}**: ${f.description}`, d),
    { kind: Dn.Markdown, value: "" }
  );
}
function ho(t) {
...
let _c = class {
  ...
  async getApiDatabase(e) {
    ...
    return i || (i = this._settings.getApiLibrary(n), this.localeToApiLibraryPromiseMap.set(n, i)), await Ec(i, e);
  }
  /**
...
const St = {
  ...
  [_.InvalidModuleUri]: "Module uri must be a text literal.",
  [_.ForInOfLoopInitializer]: "for-in loop variable declaration may not have an initializer.",
  [_.IdentifierExpected]: "'${value}' is an invalid identifier.",
  [_.InvalidEscapedReservedWord]: "Keyword cannot contain escaped characters.",
  [_.InvalidExpression]: "Invalid expression.",
  [_.InvalidFunctionIdentifier]: "'${value}' is an invalid function identifier.",
  [_.InvalidHexEscapeSequence]: "Invalid hexadecimal escape sequence.",
  [_.InvalidLeftHandSideInAssignment]: "Invalid left-hand side in assignment.",
  [_.InvalidLeftHandSideInForIn]: "Invalid left-hand side in for-in.",
  [_.InvalidTemplateHead]: "Invalid template structure.",
  [_.InvalidVariableAssignment]: "Invalid variable assignment.",
  [_.KeyMustBeString]: "Object property keys must be a word starting with a letter.",
  [_.NoFunctionInsideBlock]: "Functions cannot be declared inside of code blocks.",
  [_.NoFunctionInsideFunction]: "Functions cannot be declared inside another function.",
  [_.ModuleExportRootOnly]: "Module exports cannot be declared inside of code blocks.",
  [_.ModuleImportRootOnly]: "Module import cannot be declared inside of code blocks.",
  [_.PunctuatorExpected]: "'${value}' expected.",
  [_.TemplateOctalLiteral]: "Octal literals are not allowed in template literals.",
  [_.UnexpectedBoolean]: "Unexpected boolean literal.",
  [_.UnexpectedEndOfScript]: "Unexpected end of Arcade expression.",
  [_.UnexpectedIdentifier]: "Unexpected identifier.",
  [_.UnexpectedKeyword]: "Unexpected keyword.",
  [_.UnexpectedNull]: "Unexpected null literal.",
  [_.UnexpectedNumber]: "Unexpected number.",
  [_.UnexpectedPunctuator]: "Unexpected punctuator.",
  [_.UnexpectedString]: "Unexpected text literal.",
  [_.UnexpectedTemplate]: "Unexpected quasi '${value}'.",
  [_.UnexpectedToken]: "Unexpected token '${value}'."
};
  ...
  attachTrailingComments(e) {
    ...
    if (Ie(e) && Sa(n, e))
      for (let r = this._newComments.length - 1; r >= 0; r--) {
        ...
        Sa(s, e) && (n.trailingComments = [...n.trailingComments ?? [], s], this._newComments.splice(r, 1));
      }
    let i = [];
    if (this._newComments.length > 0)
      for (let r = this._newComments.length - 1; r >= 0; r--) {
        const s = this._newComments[r];
        Vn(s, n) ? (n.trailingComments = [...n.trailingComments ?? [], s], this._newComments.splice(r, 1)) : Vn(s, e) && (i.unshift(s), this._newComments.splice(r, 1));
      }
    if (n.trailingComments) {
      const r = n.trailingComments[0];
      Vn(r, e) && (i = [...i, ...n.trailingComments], delete n.trailingComments);
    }
  ...
  attachLeadingComments(e) {
    ...
    if (n != null) {
      ...
      for (let s = n.leadingComments.length - 1; s >= 0; s--) {
        ...
        xo(e) && !Nc(a) && (r.unshift(a), n.leadingComments.splice(s, 1));
      }
...
function Li(t, e) {
  const n = bo[t];
  return e ? n.replace(Pc, (i, r) => e[r]?.toString() ?? "") : n;
}
class Co {
  constructor(e = !1) {
  ...
  throwError(e) {
    throw e.description ??= Li(e.code, e.data), new At(e);
  }
  tolerateError(e) {
    e.description ??= Li(e.code, e.data);
    const n = new At(e);
    if (this.tolerant)
      this.recordError(n);
    else
...
const ka = {
  ...
  isIdentifierStart(t) {
    ...
    t >= 97 && t <= 122 || // a..z
    t === 92 || // \ (backslash)
    t >= 128 && ka.NonAsciiIdentifierStart.test(V.fromCodePoint(t));
  },
  isIdentifierPart(t) {
    return t === 36 || t === 95 || // $ (dollar) and _ (underscore)
    t >= 65 && t <= 90 || // A..Z
    ...
    t >= 48 && t <= 57 || // 0..9
    t === 92 || // \ (backslash)
    t >= 128 && ka.NonAsciiIdentifierPart.test(V.fromCodePoint(t));
  },
...
const tt = [[], [], []];
Ci.forEach((t) => tt[t.length - 1]?.push(t));
fo.forEach((t) => tt[t.length - 1]?.push(t));
go.forEach((t) => tt[t.length - 1]?.push(t));
mo.forEach((t) => tt[t.length - 1]?.push(t));
yc.forEach((t) => tt[t.length - 1]?.push(t));
class Eo {
  constructor(e, n) {
  ...
  throwUnexpectedToken(e = _.UnexpectedToken) {
  ...
  tolerateUnexpectedToken(e = _.UnexpectedToken) {
  ...
  scanHexEscape(e) {
    ...
    for (let r = 0; r < n; r++)
      if (!this.eof() && V.isHexDigit(this.source.charCodeAt(this.index)))
        i = i * 16 + Ba(this.source[this.index] ?? ""), this.index += 1;
      else
        return null;
  ...
  octalToDecimal(e) {
    let n = e !== "0", i = $n(e);
    return !this.eof() && V.isOctalDigit(this.source.charCodeAt(this.index)) && (n = !0, i = i * 8 + $n(this.source[this.index] ?? ""), this.index += 1, "0123".includes(e) && !this.eof() && V.isOctalDigit(this.source.charCodeAt(this.index)) && (i = i * 8 + $n(this.source[this.index] ?? ""), this.index += 1)), { code: i, octal: n };
  }
  // https://tc39.github.io/ecma262/#sec-names-and-keywords
  scanIdentifier() {
    let e;
    const n = this.index, i = this.source.charCodeAt(n) === 92 ? this.getComplexIdentifier() : this.getIdentifier();
    if (i.length === 1 ? e = S.Identifier : this.isKeyword(i) ? e = S.Keyword : i.toLowerCase() === St.Null ? e = S.NullLiteral : i.toLowerCase() === St.True || i.toLowerCase() === St.False ? e = S.BooleanLiteral : e = S.Identifier, e !== S.Identifier && n + i.length !== this.index) {
      const r = this.index;
      this.index = n, this.tolerateUnexpectedToken(_.InvalidEscapedReservedWord), this.index = r;
    }
    return {
      type: e,
      value: i,
  ...
  scanStringLiteral() {
    const e = this.index;
    let n = this.source[e];
    Na(n === "'" || n === '"', "String literal must starts with a quote"), this.index += 1;
    let i = !1, r = "";
    for (; !this.eof(); ) {
      ...
      if (s === "\\")
        if (s = this.source[this.index] ?? "", this.index += 1, !s || !V.isLineTerminator(s.charCodeAt(0)))
          switch (s) {
            ...
            case "x": {
              const a = this.scanHexEscape(s);
              a === null && this.throwUnexpectedToken(_.InvalidHexEscapeSequence), r += a;
              break;
            }
            case "n":
              r += `
...
`;
            ...
            case "x": {
              ...
              l === null && this.throwUnexpectedToken(_.InvalidHexEscapeSequence), e += l;
              break;
            }
            case "b":
              e += "\b";
            ...
            case "v":
              ...
              break;
            default:
              o === "0" ? (V.isDecimalDigit(this.source.charCodeAt(this.index)) && this.throwUnexpectedToken(_.TemplateOctalLiteral), e += "\0") : V.isOctalDigit(o.charCodeAt(0)) ? this.throwUnexpectedToken(_.TemplateOctalLiteral) : e += o;
              break;
          }
        continue;
      }
...
class Tc {
  constructor(e, n = {}, i) {
    ...
    }, this.options.comments && (this.commentHandler = new Mc()), this.errorHandler = new Co(this.options.tolerant), this.scanner = new Eo(e, this.errorHandler), this.context = {
      isAssignmentTarget: !1,
      blockContext: fe.None,
      curlyParsingType: qn.AsObject
    }, this.rawToken = {
      type: S.EOF,
      value: "",
  ...
  throwIfInvalidType(e, n, {
    ...
    i?.some((s) => e.type === s) || r?.some((s) => e.type === s) && this.throwError(_.InvalidExpression, n);
  }
  ...
  unexpectedTokenError(e = {}) {
    ...
    if (n) {
      if (!i)
        switch (n.type) {
          case S.EOF:
            i = _.UnexpectedEndOfScript;
            break;
          case S.Identifier:
            i = _.UnexpectedIdentifier;
            break;
          case S.NumericLiteral:
            i = _.UnexpectedNumber;
            break;
          case S.StringLiteral:
            i = _.UnexpectedString;
            break;
          case S.Template:
            i = _.UnexpectedTemplate;
            break;
        }
      s = n.value.toString();
    } else
      s = "ILLEGAL";
    i ??= _.UnexpectedToken, r ??= { value: s };
    const a = Li(i, r);
    if (n) {
      const c = n.start, h = n.lineNumber, d = n.start - n.lineStart + 1;
      return new At({
        code: i,
        index: c,
        line: h,
  ...
  expectPunctuator(e) {
    ...
    this.throwUnexpectedToken({ rawToken: n, code: _.PunctuatorExpected, data: { value: e } });
  }
  expectKeyword(e) {
  ...
  withBlockContext(e, n) {
    ...
    this.context.curlyParsingType = qn.Automatic;
    const s = n.call(this);
  ...
  parsePrimaryExpression() {
    ...
    switch (n.type) {
      ...
      case S.BooleanLiteral:
        return this.context.isAssignmentTarget = !1, this.readNextRawToken(), this.finalize({
          type: g.Literal,
          value: n.value.toLowerCase() === St.True,
          raw: this.getSourceValue(n),
          isString: !1,
          ...this.getItemLocation(e)
        });
  ...
  parseArrayInitializer() {
    ...
    for (; !this.matchPunctuator("]"); ) {
      ...
      this.matchPunctuator(",") ? (this.readNextRawToken(), this.throwError(_.InvalidExpression, i)) : (n.push(this.inheritCoverGrammar(this.parseAssignmentExpression.bind(this))), this.matchPunctuator("]") || this.expectPunctuator(","));
    }
  ...
  parseObjectPropertyKey() {
    ...
    switch (n.type) {
      ...
      default:
        this.throwError(_.KeyMustBeString, e);
    }
  }
  parseObjectProperty() {
  ...
  parseTemplateElement(e = !1) {
    const n = this.rawToken;
    n.type !== S.Template && this.throwUnexpectedToken({ rawToken: n }), e && !n.head && this.throwUnexpectedToken({ code: _.InvalidTemplateHead, rawToken: n });
    const i = this.captureStartMarker();
    this.readNextRawToken();
  ...
  parseLeftHandSideExpression() {
    ...
    for (; r = this.getMatchingPunctuator("([."); )
      switch (r) {
        case "(": {
          this.context.isAssignmentTarget = !1, n.type !== g.Identifier && n.type !== g.MemberExpression && this.throwError(_.IdentifierExpected, e, i);
  ...
  parseUpdateExpression() {
    const e = this.captureStartMarker();
    let n = this.getMatchingPunctuator(Ci);
    if (n) {
      this.readNextRawToken();
      const a = this.captureStartMarker(), o = this.inheritCoverGrammar(this.parseUnaryExpression.bind(this));
      return o.type !== g.Identifier && o.type !== g.MemberExpression && o.type !== g.CallExpression && this.throwError(_.InvalidExpression, a), this.context.isAssignmentTarget || this.tolerateError(_.InvalidLeftHandSideInAssignment, e), this.context.isAssignmentTarget = !1, this.finalize({
    ...
    return this.hasLineTerminator || (n = this.getMatchingPunctuator(Ci), !n) ? r : (r.type !== g.Identifier && r.type !== g.MemberExpression && this.throwError(_.InvalidExpression, i, s), this.context.isAssignmentTarget || this.tolerateError(_.InvalidLeftHandSideInAssignment, e), this.readNextRawToken(), this.context.isAssignmentTarget = !1, this.finalize({
      type: g.UpdateExpression,
      operator: n,
      argument: r,
      prefix: !1,
  ...
  parseUnaryExpression() {
    const e = this.getMatchingPunctuator(fo);
    if (e) {
  ...
  parseBinaryExpression() {
    ...
    const l = [a, i, o], c = [r];
    for (; this.rawToken.type === S.Punctuator && (r = Ma(this.rawToken.value)) > 0; ) {
      for (; l.length > 2 && r <= c[c.length - 1]; ) {
        o = l.pop();
        const f = l.pop();
        c.pop(), a = l.pop(), s.pop();
        const m = s[s.length - 1], b = Ia(m, m.lineStart);
    ...
    for (; h > 1; ) {
      ...
      if (!f)
        break;
      const m = d?.lineStart, b = Ia(f, m), D = l[h - 1];
      n = this.finalize(
        this.createBinaryOrLogicalExpression(b, D, l[h - 2], n)
      ), h -= 2, d = f;
    }
    return n;
  }
  createBinaryOrLogicalExpression(e, n, i, r) {
    const s = go.includes(n) ? g.LogicalExpression : g.BinaryExpression;
    return s === g.BinaryExpression ? { type: s, operator: n, left: i, right: r, ...this.getItemLocation(e) } : ((i.type === g.AssignmentExpression || i.type === g.UpdateExpression) && this.throwError(_.InvalidExpression, ...Ra(i)), (r.type === g.AssignmentExpression || r.type === g.UpdateExpression) && this.throwError(_.InvalidExpression, ...Ra(i)), { type: s, operator: n, left: i, right: r, ...this.getItemLocation(e) });
  }
  parseAssignmentExpression() {
    const e = this.captureStartMarker(), n = this.inheritCoverGrammar(this.parseBinaryExpression.bind(this)), i = this.captureStartMarker(), r = this.getMatchingPunctuator(mo);
    if (!r)
      return n;
    n.type !== g.Identifier && n.type !== g.MemberExpression && this.throwError(_.InvalidExpression, e, i), this.context.isAssignmentTarget || this.tolerateError(_.InvalidLeftHandSideInAssignment, e), this.matchPunctuator("=") || (this.context.isAssignmentTarget = !1), this.readNextRawToken();
  ...
  parseBlockOrObjectStatement() {
    return this.context.curlyParsingType === qn.AsObject ? this.parseObjectStatement() : this.peekAhead((e) => {
      let n = e();
  ...
  parseVariableDeclarator() {
    ...
    n || this.throwUnexpectedToken({ code: _.IdentifierExpected });
    let i = null;
    if (this.matchPunctuator("=")) {
      this.readNextRawToken();
      ...
      try {
        ...
        this.throwUnexpectedToken({ rawToken: r, code: _.InvalidVariableAssignment });
      }
    }
  ...
  parseIfStatement() {
    ...
    this.expectPunctuator(")"), (i.type === g.AssignmentExpression || i.type === g.UpdateExpression) && this.throwError(_.InvalidExpression, n, r);
  ...
  parseForStatement() {
    ...
    if (this.expectKeyword(W.For), this.expectPunctuator("("), this.matchKeyword(W.Var)) {
      ...
      this.readNextRawToken();
      const h = this.parseVariableDeclarators();
      n = this.finalize({
        type: g.VariableDeclaration,
        declarations: h,
        kind: "var",
      ...
      const d = this.matchKeyword(W.In), f = this.matchContextualKeyword(Un.Of);
      h.length === 1 && (d || f) && (h[0].init && this.throwError(_.ForInOfLoopInitializer, c), e = d ? g.ForInStatement : g.ForOfStatement, s = n);
    } else if (!this.matchPunctuator(";")) {
      const c = this.context.isAssignmentTarget, h = this.captureStartMarker();
      n = this.inheritCoverGrammar(this.parseAssignmentExpression.bind(this));
      const d = this.matchKeyword(W.In), f = this.matchContextualKeyword(Un.Of);
      d || f ? (this.context.isAssignmentTarget || this.tolerateError(_.InvalidLeftHandSideInForIn, h), n.type !== g.Identifier && this.throwError(_.InvalidLeftHandSideInForIn, h), e = d ? g.ForInStatement : g.ForOfStatement, s = n) : this.context.isAssignmentTarget = c;
    }
  ...
  parseFormalParameters() {
    ...
    if (this.expectPunctuator("("), !this.matchPunctuator(")"))
      for (; this.rawToken.type !== S.EOF; ) {
        ...
        if (n || this.throwUnexpectedToken({ rawToken: this.rawToken, code: _.IdentifierExpected }), e.push(n), this.matchPunctuator(")") || (this.expectPunctuator(","), this.matchPunctuator(")")))
          break;
      }
    return this.expectPunctuator(")"), e;
  }
  parseFunctionDeclaration() {
    (this.context.blockContext & fe.Function) === fe.Function && this.throwUnexpectedToken({ code: _.NoFunctionInsideFunction }), ((this.context.blockContext & fe.WhileLoop) === fe.WhileLoop || (this.context.blockContext & fe.ForOfLoop) === fe.ForOfLoop || (this.context.blockContext & fe.IfClause) === fe.IfClause) && this.throwUnexpectedToken({ code: _.NoFunctionInsideBlock });
    const e = this.captureStartMarker();
    this.expectKeyword(W.Function);
    const n = this.parseIdentifier();
    n || this.throwUnexpectedToken({ code: _.InvalidFunctionIdentifier });
  ...
  parseExportDeclaration() {
    this.context.blockContext !== fe.None && this.throwUnexpectedToken({ code: _.ModuleExportRootOnly });
    let e = null;
    const n = this.captureStartMarker();
    return this.expectKeyword(W.Export), this.matchKeyword(W.Var) ? e = this.parseVariableDeclaration() : this.matchKeyword("function") ? e = this.parseFunctionDeclaration() : this.throwUnexpectedToken({ code: _.InvalidExpression }), this.finalize({
  ...
  parseModuleSpecifier() {
    ...
    this.throwError(_.InvalidModuleUri, e);
  }
  parseDefaultSpecifier() {
    const e = this.captureStartMarker(), n = this.parseIdentifier();
    return n || this.throwUnexpectedToken({ code: _.IdentifierExpected }), this.finalize({
  ...
  parseImportDeclaration() {
    this.context.blockContext !== fe.None && this.throwUnexpectedToken({ code: _.ModuleImportRootOnly });
    const e = this.captureStartMarker();
    this.expectKeyword(W.Import);
    const n = this.parseDefaultSpecifier();
    this.expectContextualKeyword(Un.From);
...
class Uc {
  constructor(e, n) {
    this.errorHandler = new Co(), this.errorHandler.tolerant = n ? typeof n.tolerant == "boolean" && n.tolerant : !1, this.scanner = new Eo(e, this.errorHandler), this.trackComments = n ? typeof n.comment == "boolean" && n.comment : !1, this.buffer = [];
  }
  errors() {
...
function Ao(t, e, n) {
  return new Tc(t, e, n).parseScript();
}
function Vc(t, e, n) {
  const i = new Uc(t, e), r = [];
  let s;
  try {
    let a;
...
function qc(t, e) {
  ...
  return Ln(n.start, n.end) < 0 && (n.start = { ...n.start }, e.skipSpaces(n.start), Ln(n.start, n.end) < 0 && (n.end = { ...n.end }, e.skipSpaces(n.end, !0))), t.leadingComments?.forEach(i), t.trailingComments?.forEach(i), n;
}
function zc(t, e) {
  Ln(e.start, t.start) < 0 && (t.start = e.start), Ln(t.end, e.end) < 0 && (t.end = e.end);
}
const Wc = /\S/u;
function Wt(t = "") {
  return !Wc.test(t);
...
function Ui(t) {
  return {
    indentWidth: t?.indentWidth ?? zn.indentWidth,
    wrapColumn: t?.wrapColumn ?? zn.wrapColumn,
    quote: t?.quote ?? zn.quote
  };
}
const Oc = /^\s*/u, Hc = /\u000D\u000A|\u000D(?!\u000A)|\u000A|\u2028|\u2029/u;
class vn {
...
` : a >= r.sliceEnd ? "" : r.line.charAt(a);
  ...
  getStatementLineSeparators(e) {
    ...
    return n.length === 1 || !Wt(n.lineRecords[1].line) ? [`
`] : [`
`, `
`];
  }
...
`];
  ...
  skipSpaces(e, n = !1) {
    if (n) {
      for (; this.prevPos(e); )
        if (!Wt(this.charAt(e)) && this.nextPos(e))
          return e;
      return null;
    } else {
      for (; Wt(this.charAt(e)); )
        if (!this.nextPos(e))
          return null;
      return e;
    }
  ...
  slice(e, n) {
    ...
    return e.line === n.line ? i[0] = ct(i[0], e.column, n.column) : (i[0] = ct(i[0], e.column), i.push(ct(i.pop() ?? i[0], 0, n.column))), new vn(i);
  }
  ...
  static joinLines(e = [], n = "") {
    ...
    return e.forEach((r) => {
      if (r == null)
        return;
      const s = i.lines.pop() ?? "";
      typeof r == "string" ? i.lines.push(Pa(s, n, r)) : (i.lines.push(Pa(s, n, r.lines.slice(0)[0])), i.lines.push(...r.lines.slice(1)));
    }), i;
  }
  /**
  ...
  switch (t.type) {
    ...
    case g.LogicalExpression:
      switch (e.type) {
        ...
        case g.BinaryExpression:
        case g.LogicalExpression:
          return En[e.operator] > En[t.operator];
        default:
          return !1;
      }
    case g.Literal:
      return ya(e) && typeof t.value == "number" && e.object === t;
...
function it(t, e, n) {
  ...
  switch (t.type) {
    ...
    case g.ObjectExpression:
      return i.setupCollection(
        t.properties.map((r) => new Zc(r, t, n)),
        "CurlyBrackets"
      ), i;
    case g.VariableDeclaration:
      return i.prefix = "var ", i.setupCollection(
        t.declarations.map((r) => wo(r, t, n)),
        "None"
      ), i;
    ...
    case g.ForOfStatement:
      return i.prefix = "for ", i.setupCollection([new $i(t, e, n)], "Parentheses", ""), i;
    case g.IfStatement:
...
function wo(t, e, n) {
  return t.init ? new $i(t, e, n) : new vi(t.id, e, n);
}
function Jc(t, e, n) {
  const i = [
    ...Fn(t.left, t.operator),
    ...Fn(t.right, t.operator)
  ].map((s) => J(s, t, n)), r = new Sn(t, e, n);
...
class $e {
  constructor(e, n, i) {
    this.node = e, this.parent = n, this.options = i, this.prefix = "", this.suffix = "", this.groupingParentheses = !1, this.groupingParentheses = Xc(e, n);
  }
...
class Sn extends $e {
  ...
  getLinesLevel(e = 0) {
    return this.bracketsType !== "None" || kc(this.parent) || xo(this.parent) || (wi(this.node) || Ai(this.node)) && (wi(this.parent) || Ai(this.parent)) ? e + 1 : e;
  }
...
const Qc = /^[a-zA-Z_$][a-zA-Z_$0-9]*$/u;
class vi extends $e {
  constructor(e, n, i) {
  ...
  format() {
    ...
    if (Rc(this.parent) && this.parent.key === this.node && Do(this.node) && typeof this.node.value == "string" && Qc.test(this.node.value))
      return this.finalize(this.node.value);
    switch (this.options.quote) {
      case "auto": {
        const e = JSON.stringify(this.node.value), n = Ot(JSON.stringify(Ot(this.node.value)));
        return this.finalize(e.length > n.length ? n : e);
      }
      case "single":
        return this.finalize(Ot(JSON.stringify(Ot(this.node.value))));
      default:
...
class Fi extends $e {
  constructor(e, n, i) {
    super(e, n, i), this.node = e, this.parent = n, this.options = i, this.type = "VariableDeclaratorsPrinter", e.declarations.length === 1 ? (this.printer = wo(e.declarations[0], e, this.options), this.printer.prefix = "var ") : this.printer = it(e, n, this.options);
  }
  format(e = "singleLine", n = 0) {
    return this.finalize(this.printer.format(e, n));
  }
...
class Yc extends $e {
  constructor(e, n, i) {
    super(e, n, i), this.node = e, this.parent = n, this.options = i, this.type = "CallExpressionPrinter", this.argumentsPrinter = new Sn(e, n, i), this.argumentsPrinter.setupCollection(
      e.arguments.map((r) => J(r, e, i)),
      "Parentheses"
    ), Ne(e.callee) ? this.argumentsPrinter.prefix = e.callee.name : this.memberExpressionPrinter = new vo(e.callee, n, i);
  }
  format(e = "singleLine", n = 0) {
    ...
    let s = Vi(n, this.options) + this.leadingString.length + i.firstLineLength;
...
class Zc extends $e {
  ...
  format(e = "singleLine", n = 0) {
    ...
    if (wn(this.node.value) || wn(this.node.value)) {
...
class $i extends $e {
  constructor(e, n, i) {
    switch (super(e, n, i), this.node = e, this.parent = n, this.options = i, this.type = "OperationPrinter", this.operator = "", this.noNewLineWrapping = !1, e.type) {
      case g.AssignmentExpression:
        this.leftPrinter = J(e.left, e, this.options), this.operator = ` ${e.operator}`, this.rightPrinter = J(e.right, e, this.options), this.noNewLineWrapping = (Ne(e.left) || Do(e.left)) && (wn(e.right) || _a(e.right));
        return;
      case g.VariableDeclarator:
        this.leftPrinter = J(e.id, e, this.options), e.init && (this.operator = " =", this.rightPrinter = J(e.init, e, this.options)), this.noNewLineWrapping = wn(e.init) || _a(e.init);
        return;
      case g.ForInStatement:
      case g.ForOfStatement:
  ...
  format(e = "singleLine", n = 0) {
    ...
    if (e === "singleLine")
      return this.finalize(I.joinLines([i, r], " "));
    let s = Vi(n, this.options) + this.leadingString.length + i.maxLineLength + 1;
...
class qe {
  constructor(e, n, i) {
    this.node = e, this.parent = n, this.options = i, this.indentStatementFirstLine = !0, this.externalOffset = 0, this.options = Ui(this.options);
  }
  createStatementPrinter(e) {
    switch (e.type) {
      case g.Program:
        return new Mt(e, this.node, this.options);
      case g.ImportDeclaration:
        return new o1(e, this.node, this.options);
      case g.ExpressionStatement:
        return new s1(e, this.node, this.options);
      case g.FunctionDeclaration:
        return new Ua(e, this.node, this.options);
      case g.ExportNamedDeclaration:
        return An(e) ? new Ta(e, this.node, this.options) : new Ua(e, this.node, this.options);
      case g.BlockStatement:
        return new Mt(e, this.node, this.options);
      case g.ReturnStatement:
        return new a1(e, this.node, this.options);
      case g.VariableDeclaration:
        return new Ta(e, this.node, this.options);
      case g.IfStatement:
        return new i1(e, this.node, this.options);
      case g.ForStatement:
      case g.ForInStatement:
      case g.ForOfStatement:
      case g.WhileStatement:
        return new r1(e, this.node, this.options);
      case g.EmptyStatement:
      case g.BreakStatement:
      case g.ContinueStatement:
        return new n1(e, this.node, this.options);
      default:
...
class Mt extends qe {
  ...
  formatEmptyBlock(e = 0) {
    ...
    return n.isEmpty ? Ei(this.parent) ? this.addComments(I.appendLines([i, r]), e) : this.addComments(I.fromString("{}"), e) : this.addComments(I.appendLines([i, n, r]), e);
  }
}
class n1 extends qe {
  constructor(e, n, i) {
...
class Ta extends qe {
  constructor(e, n, i) {
    if (super(e, n, i), this.node = e, An(e))
      this.printer = new Fi(e, n, i);
    else if (An(e.declaration))
      this.printer = new Fi(e.declaration, n, i), this.printer.prefix = "export ";
    else
...
class Ua extends qe {
  constructor(e, n, i) {
    if (super(e, n, i), this.node = e, Fa(e))
      this.header = it(e, n, i), this.body = new Mt(e.body, e, i);
    else if (Fa(e.declaration))
      this.header = it(e.declaration, n, i), this.header.prefix = "export ", this.body = new Mt(e.declaration.body, e, i);
    else
      throw new Error(`invalid type: ${JSON.stringify(e)}`);
  }
  format(e = 0, n = "optimized") {
...
class i1 extends qe {
  ...
  format(e = 0, n = "optimized") {
    ...
    if (Ie(this.node.consequent)) {
      ...
      s = I.joinLines([r, " ", l]);
    } else if (i || Ei(this.node.consequent)) {
    ...
    const a = t1(this.node.consequent) || !Ie(this.node.consequent);
    let o;
    if (this.alternate.externalOffset = 0, Ie(this.node.alternate) || Ei(this.node.alternate))
...
class a1 extends qe {
  ...
  format(e = 0, n = "optimized") {
    ...
    if (!wi(this.node.argument) && !Ai(this.node.argument)) {
...
function l1(t, e = {}) {
  try {
    const n = t.getText(), i = t.positionAt(n.length), r = { start: { line: 0, character: 0 }, end: i }, s = { ...u1, ...e.formattingOptions ?? {} }, a = c1(n, s);
...
function c1(t, e) {
  if (!t)
    return "";
  const n = Ui(e), i = vn.fromString(t, n), r = i.toString(), s = Ao(r, {
    tolerant: !0,
    comments: !0
  }), a = qc(s, i);
  return s.loc.start = a.start, s.loc.end = a.end, new Mt(s, null, n).format().toString();
}
const h1 = Object.values(W), d1 = Object.values(St), f1 = ["case", "catch", "debugger", "switch", "try"], m1 = d1.map((t) => ({ label: t, kind: re.Constant })), g1 = h1.map((t) => ({ label: t, kind: re.Keyword })), p1 = [
  {
...
function Nt(t, e) {
  return e ? t.replace(b1, (n, i) => e[i].toString()) : t;
}
const qi = /[-[\]/{}()*+?.\\^$|]/gu;
function D1(t, e) {
...
function Fo(t, e) {
  return !(x1(t, e) || D1(t, e) || t.line === e.start.line && t.column < e.start.column || t.line === e.end.line && t.column > e.end.column);
}
const ee = {
  None: 0,
  Keywords: 1,
...
function C1(t, e, n, i = {}) {
  const r = t.getText({ start: { line: 0, character: 0 }, end: e }), { tokens: s, errors: a = [] } = Vc(r, { loc: !0, tolerant: !0 });
  if (a.length)
    return { isIncomplete: !1, items: [] };
  let o = s.at(s.length - 1), l = "";
  const c = I1(e);
  o?.type === "Identifier" && Fo(c, o.loc) && (l = o.value, s.pop(), o = s[s.length - 1]);
  const h = {
    variables: i.profile?.variables ?? [],
    additionalCompletionItems: (i.snippets ?? []).map((d) => ({
      kind: re.Snippet,
      insertTextMode: ge.adjustIndentation,
      insertTextFormat: me.Snippet,
      ...d
  ...
  return o?.type === "Punctuator" && o.value === "." ? A1(s, l, h) : E1(s, l, c, h);
}
function E1(t, e, n, i) {
  const r = v1(t, n);
  if (r === ee.None)
    return { isIncomplete: !1, items: [] };
  const s = y1(t), a = [], o = new RegExp(e.replaceAll(qi, "\\$&"), "i");
  return s.forEach((l, c) => {
    ...
    o.test(c) && l.overloads.forEach((h) => {
      h.disableDocumentation || a.push({ ...h.completion, sortText: Oe.functions });
    });
...
function A1(t, e, n) {
  ...
  if (a.length === 0 || !s)
    return { isIncomplete: !1, items: [] };
  let o = w1(a, e, n);
  return o || (o = L1(a, e, s, n), o) ? o : { isIncomplete: !1, items: [] };
}
function w1(t, e, n) {
  if (t.length > 1)
    return null;
  ...
  if (i.type !== "namespace")
    return { isIncomplete: !1, items: [] };
  const r = [], s = new RegExp(e.replaceAll(qi, "\\$&"), "i");
...
function L1(t, e, n, i) {
  ...
  for (; r; ) {
    const c = s.find((h) => h.name.toLowerCase() === r);
    if (!Fc(c))
      return null;
    s = c.properties ?? [], r = t.shift();
  }
  const a = [], o = new RegExp(e.replaceAll(qi, "\\$&"), "i"), l = R1(n.loc.start);
  return s.forEach((c) => {
    o.test(c.name) && c.completion && a.push(B1(l, c.completion));
  }), { isIncomplete: !1, items: a };
}
function Va(t) {
...
function v1(t, e) {
  ...
  switch (n.type) {
    ...
    default:
      return Fo(e, n.loc) ? ee.None : ee.All;
  }
}
function F1(t) {
  const e = [...t];
...
function S1(t, e) {
  for (; t.current; ) {
    ...
    e.set(i, {
      label: n,
      kind: re.Variable,
      detail: Nt(Kt.variableFormat, { name: n })
    });
    const r = t.increment();
...
function N1(t, e) {
  for (; t.current; ) {
    ...
    const n = t.current.value, i = n.toLowerCase(), r = {
      label: n,
      kind: re.Function,
      detail: Nt(Kt.functionFormat, { name: n, paramsDetail: "" }),
      insertText: `${n}()`,
      insertTextMode: ge.adjustIndentation,
      insertTextFormat: me.Snippet
    };
    ...
    for (; s && !(s.type !== "Identifier" || (a.push(s.value), s = t.increment(), !Ke(s, ","))); )
      s = t.increment();
    if (a.length) {
      r.detail = Nt(Kt.functionFormat, {
        name: r.label,
        paramsDetail: a.join(", ")
      });
      const c = a.map((h, d) => `\${${d + 1}:${h}}`).join(", ");
    ...
    t.currentIndex = o, a.forEach((c) => {
      ...
      e.set(h, {
        label: c,
        kind: re.Variable,
        detail: Nt(Kt.parameterFormat, { name: c })
      });
    });
  }
}
...
class M1 {
  ...
  validateScript(e) {
    ...
    let n = null;
    try {
      n = Ao(e, { tolerant: !0 }), this.handleErrors(n.errors), n.body.forEach(this.validateStatement), this.diagnoseIdentifiers(), this._undeclaredIdentifiersInFunctions.size > 0 && this._undeclaredIdentifiersInFunctions.forEach((i) => {
        for (const r of i)
  ...
  validateForStatement(e) {
    An(e.init) ? this.inBlock(this.validateStatement, e.init) : this.validateExpression(e.init), this.validateExpression(e.update), this.validateExpression(e.test), ht(e.body) && this.logDiagnostic(e.body.loc, {
  ...
  validateCallExpression(e) {
    ...
    if (this._isInCallExpression = !0, this.validateExpression(e.callee), Ne(e.callee)) {
      ...
      if (!r && s) {
        const a = T1(s, e.arguments.length);
  ...
  logReservedKeywordsConflict(e) {
    ...
    f1.includes(n) && this.logDiagnostic(e.loc, {
...
function ht(t) {
  return Ti(t) || Ie(t) && !t.body.length;
}
function T1(t, e) {
...
const U1 = /\B@\w+/gu;
function V1(t = "") {
  return Array.from(t.matchAll(U1), (e) => e[0].slice(1));
}
class $1 {
  configure(e) {
    this.apiService = new _c(e);
  }
  async doValidation(e, n) {
    if (!this.apiService)
      return [];
    const i = await this.apiService.getApiDatabase(n), { profile: { variables: r = [] } = {} } = n, { diagnostics: s } = new M1(i, r).validateScript(e.getText());
    return s;
  }
...
class Pt {
  ...
  update(e, n) {
    for (const i of e)
      if (Pt.isIncremental(i)) {
        const r = yo(i.range), s = this.offsetAt(r.start), a = this.offsetAt(r.end);
        ...
        const h = qa(i.text, !1, s);
        if (l - o === h.length)
          for (let f = 0, m = h.length; f < m; f++)
            c[f + o + 1] = h[f];
        else
          h.length < 1e4 ? c.splice(o + 1, l - o, ...h) : this._lineOffsets = c = c.slice(0, o + 1).concat(h, c.slice(l + 1));
        const d = i.text.length - (a - s);
        if (d !== 0)
          for (let f = o + 1 + h.length, m = c.length; f < m; f++)
            c[f] = c[f] + d;
      } else if (Pt.isFull(i))
  ...
  ensureBeforeEOL(e, n) {
    for (; e > n && _o(this._content.charCodeAt(e - 1)); )
      e--;
    return e;
  }
  get lineCount() {
...
(function(t) {
  ...
  t.update = n;
  function i(r, s) {
    const a = r.getText(), o = yi(s.map(q1), (h, d) => {
    ...
    const c = [];
    for (const h of o) {
      const d = r.offsetAt(h.range.start);
      if (d < l)
        throw new Error("Overlapping edit");
      d > l && c.push(a.substring(l, d)), h.newText.length && c.push(h.newText), l = r.offsetAt(h.range.end);
    }
...
function qa(t, e, n = 0) {
  ...
  for (let r = 0; r < t.length; r++) {
    ...
    _o(s) && (s === 13 && r + 1 < t.length && t.charCodeAt(r + 1) === 10 && r++, i.push(n + r + 1));
  }
  return i;
}
function _o(t) {
...
class z1 {
  ...
  constructor(e, { languageId: n }) {
    this._ctx = e, this._languageId = n, this._languageService = new $1(), this._languageService.configure({ getApiLibrary: this._ctx.host.getApiLibrary });
  }
  async doValidation(e, n) {
  ...
  _getTextDocument(e) {
    ...
    for (const i of n)
      if (i.uri.toString() === e)
        return _i.create(e, this._languageId, i.version, i.getValue());
    return null;
  }
}
