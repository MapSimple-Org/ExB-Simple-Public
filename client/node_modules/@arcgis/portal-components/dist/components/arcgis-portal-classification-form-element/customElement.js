...
import { LitElement as u, createEvent as b, nothing as a } from "@arcgis/lumina";
...
class v extends u {
  ...
  renderCheckbox() {
    return l(this.attribute, "checkbox") ? i`<calcite-label class="spacing-y" .for=${this.attributeKey} layout=inline><calcite-checkbox .checked=${this.value} id=${this.attributeKey ?? a} @calciteCheckboxChange=${(e) => this.arcgisAttributeValueChange.emit({ [this.attributeKey]: e.target.checked })} .required=${this.attribute.isRequired ?? !1} .status=${this.validationMsg ? "invalid" : "valid"}></calcite-checkbox>${this.attribute.label}</calcite-label>` : null;
  }
  renderDateInput() {
    return l(this.attribute, "date") ? i`<calcite-label .for=${this.attributeKey}>${this.attribute.label}<calcite-input-date-picker id=${this.attributeKey ?? a} .name=${this.attribute.label} @calciteInputDatePickerChange=${(e) => this.isConnected && this.arcgisAttributeValueChange.emit({ [this.attributeKey]: e.target.value })} overlay-positioning=fixed .required=${this.attribute.isRequired} .status=${this.validationMsg ? "invalid" : "valid"} validation-icon=exclamation-mark-triangle .validationMessage=${this.validationMsg} .value=${this.value}></calcite-input-date-picker></calcite-label>` : null;
  }
  renderTextInput() {
    if (!l(this.attribute, "text"))
      return null;
    const e = this.attribute.label, t = this.attribute.description;
    return i`<calcite-label .for=${this.attributeKey}><div class="portal-classification-form-element-tooltip">${e}${this.attribute.labelTooltip && i`<div><calcite-icon scale=s icon=information .id=${`tooltip-button-${this.attribute.bannerLabel}`} text-label=${this.messages.information ?? "Information" ?? a}></calcite-icon><calcite-tooltip .referenceElement=${`tooltip-button-${this.attribute.bannerLabel}`} placement=right>${this.attribute.labelTooltip}</calcite-tooltip></div>` || ""}</div><calcite-input-text id=${this.attributeKey ?? a} .label=${e} .name=${this.attributeKey} @calciteInputTextChange=${(s) => this.arcgisAttributeValueChange.emit({ [this.attributeKey]: s.target.value })} .placeholder=${t} .required=${this.attribute.isRequired} .status=${this.validationMsg ? "invalid" : "valid"} validation-icon=exclamation-mark-triangle .validationMessage=${this.validationMsg} .value=${this.value}></calcite-input-text></calcite-label>`;
  }
  renderSingleSelect() {
  ...
  _renderSingleSelectComboboxOptions() {
    if (!Array.isArray(this.renderingInfo.valueExpressionResult))
      return;
    const e = this.value;
    return c(this.renderingInfo.valueExpressionResult, (t) => t.label, (t) => i`<calcite-combobox-item .selected=${e === t.value} text-label=${t.label ?? a} .value=${t.value} data-testid=${`single-combobox-item-${t.value}`}></calcite-combobox-item>`);
  }
  ...
  renderMultiSelect() {
    return l(this.attribute, "multi-select") ? i`<calcite-label .for=${this.attributeKey}>${this.attribute.label}<calcite-combobox id=${this.attributeKey ?? a} .label=${this.attribute.label} .name=${this.attributeKey} @calciteComboboxChange=${(e) => {
      ...
      this.arcgisAttributeValueChange.emit({
        [this.attributeKey]: t ? Array.isArray(t) ? t : [t] : void 0
      });
    }} .placeholder=${this.messages.selectoptions ?? "Select option(s)"} .required=${this.attribute.isRequired} .status=${this.validationMsg ? "invalid" : "valid"} validation-icon=exclamation-mark-triangle .validationMessage=${this.validationMsg} .value=${this.value}>${this._renderComboboxOptions()}</calcite-combobox></calcite-label>` : null;
  }
  ...
  renderMultiGroupedSelectNested() {
    ...
    }} .placeholder=${this.messages.selectoptions ?? "Select option(s)"} .required=${this.attribute.isRequired} .status=${this.validationMsg ? "invalid" : "valid"} validation-icon=exclamation-mark-triangle .validationMessage=${this.validationMsg} .value=${this.value}>${this._renderGroupedComboboxOptions()}</calcite-combobox></calcite-label>` : null;
  }
  render() {
    switch (this.attribute.uiElement) {
      case "checkbox":
      ...
      case "multi-grouped-select-nested":
        return this.renderMultiGroupedSelectNested();
      case "single-select":
        return this.renderSingleSelect();
      case "text":
        return this.renderTextInput();
      default:
        return i`<div>Not Implemented Yet.</div>`;
    }
  }
}
