...
test.describe('QuerySimple Bug Reproduction (r017.40)', () => {
  ...
  const WIDGET_1_ID = 'widget_12'; // QuerySimple (Main)
  ...
  test('should reproduce Sticky Expansion and Dirty Hash during rapid switching', async ({ page }) => {
    ...
    // Switch back to Query tab to check the form value
    console.log('🖱️ Switching back to Query tab to check form value');
    const queryTab = widget.locator('button[data-testid="query-tab"], button:has-text("Query")').first();
    ...
    // Remove isVisible: true filter as it might be hidden in some states
    ...
    // --- STEP 3: TEST QUERY PARAMETER SUPPORT (?pin=) ---
    console.log(`🚀 Step 3: Testing Query Parameter support (?pin=${PIN_1})`);
    await page.goto(`${baseURL}${APP_URL}?pin=${PIN_1}&debug=all`, { waitUntil: 'networkidle' });
    ...
    const inputStep3 = widget.locator('input.jimu-input-base, input:not([type="hidden"])').first();
    await inputStep3.waitFor({ state: 'attached', timeout: 10000 });
    const inputValueStep3 = await inputStep3.inputValue();
    console.log(`🔍 Step 3 Search Value: ${inputValueStep3} (Expected: ${PIN_1})`);
    
    expect(inputValueStep3, 'Query Parameter support failed!').toBe(PIN_1);
  });
});
