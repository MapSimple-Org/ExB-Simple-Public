...
describe('results-management-utils unit tests', () => {
  ...
  describe('removeRecordsFromOriginSelections', () => {
    ...
    it('should group records by origin DS and update selections', () => {
      ...
      // Verify selection change message was published
      expect(mockMessageManager.publishMessage).toHaveBeenCalled();
      // r027.014: Message is constructed with empty records array (ID-based selection since r027.010)
      expect(DataRecordsSelectionChangeMessage).toHaveBeenCalledWith('widget_1', [], ['origin_ds_1']);
    });
