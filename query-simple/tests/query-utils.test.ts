...
describe('query-utils unit tests', () => {
  describe('combineFields', () => {
    ...
    // r028.106: legacy single-brace {field} content tokens must be extracted too.
    // CustomTemplate configs authored with {field} (not {{field}}) were under-fetching.
    it('should extract legacy single-brace {field} tokens from contentExpression', () => {
      const result = combineFields(null as any, ' Title: {NAME}', 'OBJECTID', '![img](url/{PHOTO}) {BLURB} and {LENGTH}');
      expect(result).toContain('NAME');    // single-brace title (already supported)
      expect(result).toContain('PHOTO');   // single-brace content (the fix)
      expect(result).toContain('BLURB');
      expect(result).toContain('LENGTH');
      expect(result).toContain('OBJECTID');
    });

    it('should not double-count a {{field}} content token as single-brace {field}', () => {
      const result = combineFields(null as any, '', 'OBJECTID', '{{ADDRESS}} and {CITY}');
      expect(result).toContain('ADDRESS'); // double-brace
      expect(result).toContain('CITY');    // single-brace
      expect(result).toContain('OBJECTID');
      expect(result.length).toBe(3);       // ADDRESS not also matched as single-brace
    });
  });
  ...
  describe('generateQueryParams - PopupSetting mode', () => {
    ...
    it('should use visible popup fields for PopupSetting mode', () => {
      ...
      const params = generateQueryParams(mockDS, { sql: '1=1' } as any, null, mockQueryItem as any, 1, 10);
